...
macro_rules! float_impl {
    (
        ...
        impl Float for $ty {
            ...
            // Use the macro's declared width directly. `Self::BITS` now also names an
            // inherent primitive constant, which triggers `unstable_name_collisions`
            // and prevents workspace `-D warnings` gates from reaching dependents.
            const SIGN_MASK: Self::Int = 1 << ($bits - 1);
