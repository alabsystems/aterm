...
macro_rules! int_impl {
    ($ity:ty, $uty:ty) => {
        ...
        impl Int for $ity {
            ...
            fn abs(self) -> Self {
                // aterm-trust: `self.abs()` panics/overflows at iN::MIN (no +2^(N-1) in iN);
                // use wrapping_abs() which is overflow-free and identical for every non-MIN
                // value. libm never feeds iN::MIN through this path, so valid-input behavior
                // is unchanged while the panic/overflow edge becomes provably safe.
                self.wrapping_abs()
            }
