...
impl Repr {
    ...
    fn as_str(&self) -> &str {
        match self {
            ...
            Repr::Inline { len, buf } => {
                // aterm-trust: `len` is an `InlineSize` enum bounded by `INLINE_CAP`
                // (== `buf.len()`) by construction, but Trust treats the enum→usize
                // cast opaquely and cannot prove `len <= buf.len()` in isolation, so
                // the raw `&buf[..len]` slice is flagged as a potential OOB.
                // Clamp the index to `buf.len()` and use the checked `get(..len)`
                // accessor so the slice is provably in-bounds (no panic/OOB edge).
                // Both are no-ops for valid inputs (len is always <= INLINE_CAP);
                // the `None`/empty fallback is unreachable for valid inputs.
                let len = (*len as usize).min(buf.len());
                let buf = buf.get(..len).unwrap_or(&[]);
...
impl Writer {
    ...
    fn push_str(&mut self, s: &str) {
        ...
        if self.len <= INLINE_CAP {
            ...
            // aterm-trust: use `checked_add` instead of `self.len += s.len()`. The
            // outer guard only bounds `self.len <= INLINE_CAP`, so the raw `+` can
            // overflow `usize` for an arbitrary `s`, which Trust uses as a refuting
            // counterexample (the wrapped length could spuriously re-pass the inner
            // `<= INLINE_CAP` guard). An overflowing total cannot fit inline, so
            // treating overflow as "too big for the stack" preserves behavior for
            // all valid inputs (where `old_len + s.len()` never overflows).
            let new_len = old_len.checked_add(s.len());
            self.len = new_len.unwrap_or(usize::MAX);

            // if the new length will fit on the stack (even if it fills it entirely)
            if self.len <= INLINE_CAP {
                // aterm-trust: inside this guard `old_len <= self.len <= INLINE_CAP
                // == self.inline.len()`, but Trust treats the cast/relation opaquely
                // and flags `self.inline[old_len..self.len]` as a potential OOB. Use
                // the checked `get_mut(old_len..self.len)` accessor so the
                // destination slice is provably in-bounds. The `if let` arm always
                // matches for valid inputs (len fits, old_len <= new len), so this is
                // a no-op for valid inputs; the non-matching edge is unreachable.
                if let Some(dst) = self.inline.get_mut(old_len..self.len) {
            ...
            // aterm-trust: clamp the slice end with `get(..old_len)` (checked
            // accessor) so the source slice is provably in-bounds; `old_len` is the
            // pre-push length and is always `<= INLINE_CAP == self.inline.len()`.
            // SAFETY: inline data is guaranteed to be valid utf8 for `old_len` bytes
            unsafe {
                self.heap
                    .as_mut_vec()
                    .extend_from_slice(self.inline.get(..old_len).unwrap_or(&[]));
            }
        }
