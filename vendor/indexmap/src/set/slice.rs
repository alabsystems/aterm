...
//
// The std `SliceIndex` impls behind the bare `slice[range]` accesses are what
// Trust refutes at Level 0: the `RangeInclusive`/`RangeToInclusive` impls compute
// `end + 1` unconditionally (overflowing `usize` when `end == usize::MAX`), and
// even the plain `Range`/`RangeFrom`/`RangeTo` impls perform an access whose
// `start <= end <= len` precondition Trust cannot discharge for an arbitrary
// caller-supplied range.
//
// `crate::util::simplify_range` normalizes ANY `RangeBounds<usize>` into a
// validated half-open `start..end` (`start <= end <= len`), panicking with the
// exact std "out of range" / "start should be <= end" messages on invalid input,
// and only ever computing `i + 1` under the guard `i < len` (so `+ 1` cannot
// overflow). After normalization we use the *checked* `get` accessor; for any
// input that survives `simplify_range` the range is in bounds so `get` returns
// `Some` and the `unwrap_or_else` arm is unreachable. Valid-input behavior is
// unchanged; the previous overflow / unprovable out-of-bounds edges become the
// same panic outcome with no arithmetic overflow.
#[track_caller]
fn index_entries<T, R>(entries: &[Bucket<T>], range: R) -> &[Bucket<T>]
where
    R: RangeBounds<usize>,
{
...
macro_rules! impl_index {
    ($($range:ty),*) => {$(
        ...
        impl<T> Index<$range> for Slice<T> {
            ...
            fn index(&self, range: $range) -> &Self::Output {
                Slice::from_slice(index_entries(&self.entries, range))
            }
        }
    )*}
}
