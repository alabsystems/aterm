...
// aterm-trust: overflow-free, provably-in-bounds slice range access.
//
// The std `SliceIndex` impls used by the bare `slice[range]` accesses below are
// what Trust refutes at Level 0:
//   * `RangeInclusive`/`RangeToInclusive` compute `end + 1` unconditionally to
//     form the equivalent half-open range, which overflows `usize` when
//     `end == usize::MAX`.
//   * even the plain `Range`/`RangeFrom`/`RangeTo` impls perform an in-bounds
//     access whose `start <= end <= len` precondition Trust cannot discharge from
//     an arbitrary caller-supplied range.
//
// `crate::util::simplify_range` already normalizes ANY `RangeBounds<usize>` into a
// validated half-open `start..end` with `start <= end <= len`, panicking with the
// exact same "out of range" / "start should be <= end" messages that the std
// indexing impls produce on invalid input. Crucially it only ever computes `i + 1`
// under the guard `i < len`, so `i < len <= isize::MAX` and the `+ 1` cannot
// overflow — there is no inclusive-range overflow edge here at all.
//
// After normalization the access is `entries.get(start..end)`, the *checked*
// accessor. For any input that survives `simplify_range` the range is in bounds,
// so `get` returns `Some`; the `unwrap_or_else` arm is unreachable. This preserves
// behavior for every valid input and turns the previous overflow / unprovable
// out-of-bounds edges into the same panic outcome with no arithmetic overflow.
#[track_caller]
fn index_entries<K, V, R>(entries: &[Bucket<K, V>], range: R) -> &[Bucket<K, V>]
where
    R: RangeBounds<usize>,
{
    let range = crate::util::simplify_range(range, entries.len());
    entries
        .get(range)
        .unwrap_or_else(|| unreachable!("simplify_range guarantees an in-bounds range"))
}

#[track_caller]
fn index_entries_mut<K, V, R>(entries: &mut [Bucket<K, V>], range: R) -> &mut [Bucket<K, V>]
where
    R: RangeBounds<usize>,
{
    let range = crate::util::simplify_range(range, entries.len());
...
macro_rules! impl_index {
    ($($range:ty),*) => {$(
        impl<K, V, S> Index<$range> for IndexMap<K, V, S> {
            ...
            // aterm-trust: route through `index_entries` (simplify_range + checked
            // get) instead of the bare `self.as_entries()[range]` slice index.
            fn index(&self, range: $range) -> &Self::Output {
                Slice::from_slice(index_entries(self.as_entries(), range))
            }
        }

        impl<K, V, S> IndexMut<$range> for IndexMap<K, V, S> {
            // aterm-trust: route through `index_entries_mut` (simplify_range +
            // checked get_mut) instead of the bare slice index.
            fn index_mut(&mut self, range: $range) -> &mut Self::Output {
                Slice::from_mut_slice(index_entries_mut(self.as_entries_mut(), range))
            }
        }
        ...
        impl<K, V> Index<$range> for Slice<K, V> {
            ...
            // aterm-trust: route through `index_entries` instead of `self.entries[range]`.
            fn index(&self, range: $range) -> &Self {
                Self::from_slice(index_entries(&self.entries, range))
            }
        }

        impl<K, V> IndexMut<$range> for Slice<K, V> {
            // aterm-trust: route through `index_entries_mut` instead of `self.entries[range]`.
            fn index_mut(&mut self, range: $range) -> &mut Self {
                Self::from_mut_slice(index_entries_mut(&mut self.entries, range))
            }
        }
    )*}
}
