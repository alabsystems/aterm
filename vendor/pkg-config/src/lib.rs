...
impl Library {
    ...
    fn parse_libs_cflags(&mut self, name: &str, output: &[u8], config: &Config) {
        ...
        // aterm-trust: `&arg[0..2]`/`&arg[2..]` panic if byte index 2 is not a UTF-8
        // char boundary (the `len() > 2` filter alone does not prove this). Use
        // `split_at_checked(2)`, which is panic-free: it yields `None` when the string
        // is shorter than 2 bytes OR when index 2 falls inside a multi-byte char, and
        // `filter_map` drops those. Behavior is unchanged for the real >=2-char ASCII
        // flags ("-L"/"-l"/"-I"/"-D"/"-F"/"-u"), whose 2nd byte is always a boundary.
        let parts = words
            .iter()
            .filter(|l| l.len() > 2)
            .filter_map(|arg| arg.split_at_checked(2));
