...
impl<T> Offset for &[T] {
    ...
    fn offset_from(&self, start: &Self) -> usize {
        ...
        // aterm-trust: ZSTs have size_of::<T>() == 0, which would make the
        // original pointer-difference division divide by zero. For a ZST every
        // element occupies the same address, so the byte offset is always 0 and
        // there is no meaningful element offset; return 0. Guarding here removes
        // the provable divide-by-zero on the generic-over-T path.
        if core::mem::size_of::<T>() == 0 {
            return 0;
        }
        // aterm-trust: For `&[T]`, the contract requires `self` to be a tail of
        // `start` (asserted above), so the element offset is exactly
        // `start.len() - self.len()`. Computing it from the lengths instead of
        // the raw pointer subtraction avoids the underflow (and the division)
        // entirely: `saturating_sub` proves no underflow, and for valid inputs
        // (self a tail of start => self.len() <= start.len()) it yields the
        // identical result as the original pointer arithmetic.
        (*start).len().saturating_sub((*self).len())
    }
}
