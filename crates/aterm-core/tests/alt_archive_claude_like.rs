...
//!
//! The `#[ignore]`d timing tests report the archive's cost per committed frame at
//! 63x138, and its cost on main-screen output (where it commits nothing):
//! `targo --unverified test --release -p aterm-core --test alt_archive_claude_like -- --ignored --nocapture`
...
/// Cost of the archive per COMMITTED frame on a 63x138 Claude-like stream: each
/// frame is processed by two terminals, archive off and on, alternating which
/// goes first; the per-frame difference is the archive's cost (row extraction,
/// the differ, the ESU matcher). Prints p50/p90 of that difference and of the
/// absolute on/off frame times.
#[test]
#[ignore = "timing; run with --release -- --ignored --nocapture"]
fn timing_per_committed_frame_63x138() {
    let layout = Layout {
        rows: 63,
        cols: 138,
    };
    ...
    for k in steps(frames) {
        starts.push(starts.last().copied().unwrap_or(0) + k);
    }
    let screens: Vec<Vec<String>> = starts
        .iter()
        .enumerate()
        .map(|(tick, &s)| layout.screen(s, tick))
        .collect();
    let mut chunks = Vec::with_capacity(screens.len());
    for i in 0..screens.len() {
        ...
        paint(
            &mut v,
            i.checked_sub(1).map(|p| screens[p].as_slice()),
            &screens[i],
            i == 0 || i % 7 == 6,
        );
        chunks.push(v);
    }
    let mut on = term(&layout);
    let mut off = Terminal::new(layout.rows as u16, layout.cols as u16);
    off.set_alt_archive_enabled(false);
    let (mut t_on, mut t_off, mut diff) = (Vec::new(), Vec::new(), Vec::new());
    for (i, c) in chunks.iter().enumerate() {
        let time = |t: &mut Terminal| {
    ...
    assert!(
        on.alt_archive().len() > 1000,
        "the stream must actually scroll"
    );
    for v in [&mut t_on, &mut t_off, &mut diff] {
        v.sort_by(f64::total_cmp);
    }
    println!(
        "alt_archive 63x138: {} committed frames, archived {} rows",
        diff.len(),
        on.alt_archive().len()
    );
    println!(
        "  frame with archive   p50 {:.1} us  p90 {:.1} us",
        percentile(&t_on, 0.5),
        percentile(&t_on, 0.9)
    );
    println!(
        "  frame without        p50 {:.1} us  p90 {:.1} us",
        percentile(&t_off, 0.5),
        percentile(&t_off, 0.9)
    );
    println!(
        "  archive cost/frame   p50 {:.1} us  p90 {:.1} us",
        percentile(&diff, 0.5),
        percentile(&diff, 0.9)
    );
}

/// The archive's cost on MAIN-screen output, where it commits nothing: plain
/// text, SGR-heavy listings and main-screen 2026 frames, fed at 4 KiB and
/// 64 KiB per read, archive on vs off (best of 5). The matcher's per-byte scan
/// rides on every byte the terminal receives, so this is the number that says
/// what "on by default" costs a session that never enters the alt screen
/// (round-7 review: +14-30% before the matcher learned the main screen only
/// needs an alt-screen enter).
#[test]
#[ignore = "timing; run with --release -- --ignored --nocapture"]
fn timing_main_screen_overhead() {
    fn best(data: &[u8], chunk: usize, on: bool) -> f64 {
        let mut best = f64::MAX;
        for _ in 0..5 {
            let mut t = Terminal::new(63, 138);
            t.set_alt_archive_enabled(on);
            let clock = fixed_clock();
            let s = std::time::Instant::now();
            for c in data.chunks(chunk) {
                t.process_at(c, clock);
            }
    ...
    for i in 0..400_000 {
        plain.extend_from_slice(
            format!("line {i:08} the quick brown fox jumps over the lazy dog\r\n").as_bytes(),
        );
    }
    let mut sgr = Vec::new();
    for i in 0..200_000 {
        sgr.extend_from_slice(
            format!(
                "\x1b[1;34mdrwxr-xr-x\x1b[0m \x1b[32m{i}\x1b[0m \x1b[38;5;208mfile\x1b[39m\x1b[K\r\n"
            )
            .as_bytes(),
        );
    }
    ...
    for i in 0..20_000 {
        sync.extend_from_slice(b"\x1b[?2026h\x1b[?25l");
        for r in 0..6 {
            sync.extend_from_slice(
                format!("\x1b[{};1H row {i} {r} streaming text here\x1b[K", 50 + r).as_bytes(),
            );
        }
        sync.extend_from_slice(b"\x1b[?25h\x1b[?2026l");
    }
    for (name, data) in [("plain", &plain), ("sgr", &sgr), ("main-2026", &sync)] {
        for chunk in [4096usize, 65536] {
            let off = best(data, chunk, false);
            let on = best(data, chunk, true);
            println!(
                "main screen {name:10} chunk={chunk:6} bytes={:9} off={:.2}ms on={:.2}ms overhead={:+.1}%",
                data.len(),
                off * 1e3,
                on * 1e3,
                (on / off - 1.0) * 100.0
