...
impl TerminalHandler<'_> {
    ...
    ///
    /// `csi_dispatch_table.rs` is generated by `tools/csi-hash-gen`, whose hash
    /// parameters were brute-forced and proven collision-free; a sequence
    /// recognized after that search was fixed lands here instead of forcing a
    /// re-solve. This is the ONE fallback both dispatch paths share, so a
    /// sequence added for the intermediate form cannot go missing from the
    /// no-intermediate form (or the reverse) — a whole-table regen would fold
    /// every arm below back into `CSI_DISPATCH_TABLE` at once.
    ///
