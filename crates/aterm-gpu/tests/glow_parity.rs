...
use aterm_render::{
    BeamClip, BeamVertex, GlowQuad, HaloMode, RainHalo, Renderer, Theme, WindowCpu, comet_beam,
    premul_rgb,
};
...
fn backends(px: f32, theme: Theme) -> Option<(Renderer, aterm_gpu::GpuRenderer)> {
    ...
    // absorb the same sub-pixel offset, so the additive parity this test checks is
    ...
    FORCE_FONTDUE.call_once(|| {
        // SAFETY: set once, before any renderer in this test binary is built; every
        // glow-parity test wants the same deterministic value.
