...
pub const VERBS: &[VerbSpec] = &[
    ...
    v(
        ...
        Status,
        App,
        "render/latency counters [reset|percentiles] - percentiles: p50/p95/p99 input->application-present-return / output->application-present-return / frame-render distributions; plain line carries max_frame_gap_ms= (worst successful-present-return gap since reset), rust_main_to_first_present_ms= and rust_main_to_first_visible_ms= (Rust entry->the FIRST window's actual reveal — time-to-visible; on a warm Windows launch the reveal precedes the backend join, so it runs well under first-present) plus startup_phase_schema=1/startup_phase_valid= and eight exclusive startup_*_ms phases (router, GUI prepare, winit dispatch, initial surface attach, successful-redraw wait/compose/surface/finalize); startup_attach_schema=1/startup_attach_valid= drills the attach parent into dispatch/prepare/window-create/window-setup/backend-finalize/chrome-geometry/surface-create/finish, and the line ends first_present_ms= (compatibility GUI main_entry->the same successful-present publication; dyld/compositor/scanout unobserved) first_visible_ms= (GUI main_entry->the same reveal instant)",
    ),
    // drive input
    v(
        "turn",
