...
/// cross-import (mirrors `cursor_fireball::push_rect`).
...
/// The WATER ramp, `t` 0 (deep-sea abyss) → 1 (foam crest) — the same ocean
/// palette as the aurora's fluid wake and droplet splash (kept local, no
/// cross-import, mirroring `cursor_fireball::fire_ramp`). Saturated and
/// green-leaning through the midband — rich blue-green WATER, never pale ice.
