...
use aterm_spec::derive::{Model, native_config_transaction_model};
...
/// independent stale-Trail and stale-Nyan negative controls prove the aggregate
/// consistency predicate is not vacuous.
#[test]
fn config_snapshot_catalog_is_atomic_across_patch_external_and_cross_view_delivery() {
    ...
    let patched = match service.patch(ConfigPatchRequest {
        ...
        edits: vec![
            ...
            ConfigKeyEdit {
                key: "cursor_nyan_sprite".to_string(),
                expected: ExpectedValue::Exact(Some(sprites.sources[0].clone())),
                value: Some(sprites.sources[1].clone()),
            },
        ],
    }) {
    ...
    assert!(
        !Arc::ptr_eq(&initial.assets, &patched.assets),
        "each admitted generation owns one outer asset catalog"
    );
    assert!(!Arc::ptr_eq(
    ...
    app.config_assets = Arc::clone(&external.assets);
    ...
    assert_eq!(stale_trail_state["trail_generation"], 0);
    ...
    assert_eq!(stale_nyan_state["nyan_generation"], 0);
