...
//! * [`phase::worker_phase`] — busy / prompt / idle / question, from one read.
//! * [`run::Session`] — `await-turn` and `supervise`, over the control verbs.
//!
...
pub use phase::{Phase, is_placeholder, worker_phase};
pub use prompt::{Prompt, PromptKind, parse_prompt};
pub use run::{
    Caps, Ctl, CtlReply, EXIT_TIMEOUT, Session, SuperviseOpts, Turn, render_phase, render_result,
};
