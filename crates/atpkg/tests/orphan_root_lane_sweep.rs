...
fn a_launchd_helper_outliving_a_killed_root_lay_is_stopped_before_its_debris_is_swept() {
    ...
    // `JOB_STEMS`, and `ViewJob::Root` is submitted under it) beneath the pid of an owner
    // that dies before the sweep — what a killed pass leaves registered.
    ...
    // Its owner dies only now, with the helper proven laying: the orphan from here on.
    drop(owner);
