...
//! shims (`claude`, `codex` — [`crate::stub::AGENT_PROGRAMS`]), is PREPENDED as a separate
//! element (`ATPKG_AGENTS`), so the managed copy of a coding agent is the one that runs
//! ahead of a vendor's native install or a brew cask. `bin/` stays last; the deny-list
...
/// and `agents_dir` (PREPENDED — the agent programs' front-of-PATH shims, module doc).
...
pub fn hook_files(bin_dir: &Path, agents_dir: &Path) -> Vec<(String, String)> {
    ...
    // POSIX (zsh + bash): the agents dir is an idempotent PREPEND, the managed bin/ an
    // idempotent APPEND — two elements, two variables, so neither guard can mask the
    // other; quoted for a space in "Application Support".
    let posix = format!(
        "# Generated by atpkg -- DO NOT EDIT (rewritten on every install/update).\n\
         __atpkg_agents=\"{agents}\"\n\
         case \":$PATH:\" in *\":$__atpkg_agents:\"*) ;; *) export PATH=\"$__atpkg_agents:$PATH\" ;; esac\n\
         export ATPKG_AGENTS=\"$__atpkg_agents\"\n\
         unset __atpkg_agents\n\
         __atpkg_bin=\"{bin}\"\n\
         case \":$PATH:\" in *\":$__atpkg_bin:\"*) ;; *) export PATH=\"$PATH:$__atpkg_bin\" ;; esac\n\
         export ATPKG_BIN=\"$__atpkg_bin\"\n\
         unset __atpkg_bin\n"
    );
    // fish: prepend == place before $PATH, append == after; fish `set` syntax, no POSIX
    // export.
    let fish = format!(
        "# Generated by atpkg -- DO NOT EDIT (rewritten on every install/update).\n\
         set -l __atpkg_agents \"{agents}\"\n\
         if not contains $__atpkg_agents $PATH; set -gx PATH $__atpkg_agents $PATH; end\n\
    ...
    // the POSIX hooks above. Same shape as the POSIX policy (agents prepended, bin/ appended,
    // each idempotent) using `[System.IO.Path]::PathSeparator` (';' on Windows, ':' on Unix),
    // so it is correct wherever pwsh runs.
    ...
    let powershell = format!(
        "# Generated by atpkg -- DO NOT EDIT (rewritten on every install/update).\n\
         ...
         if (($env:PATH -split [regex]::Escape($__atpkg_sep)) -notcontains $__atpkg_agents) {{ $env:PATH = \"$__atpkg_agents$__atpkg_sep$env:PATH\" }}\n\
         ...
         Remove-Variable __atpkg_agents, __atpkg_bin, __atpkg_sep\n"
    );
    vec![
...
mod tests {
    ...
    #[test]
    fn powershell_dialect_appends_bin_and_prepends_agents_idempotently_via_platform_separator() {
        let files = hook_files(Path::new("/p/bin"), Path::new("/p/agents"));
        ...
        assert!(
            ps.contains("$env:PATH = \"$__atpkg_agents$__atpkg_sep$env:PATH\""),
            "agents/ is PREPENDED (the one exception)"
        );
        assert!(
            ps.contains("-notcontains $__atpkg_bin") && ps.contains("-notcontains $__atpkg_agents"),
            "idempotency guards, one per element"
        );
    ...
    /// decision 2026-09-10 — is PREPENDED as its own element with its own guard, exported
    /// as `ATPKG_AGENTS` beside `ATPKG_BIN`.
    #[test]
    fn posix_dialects_append_bin_and_prepend_only_the_agents_dir() {
        let files = hook_files(Path::new("/p/bin"), Path::new("/p/agents"));
        for (name, body) in &files {
            if name.ends_with(".zsh") || name.ends_with(".bash") {
                ...
                assert!(
                    body.contains("export PATH=\"$__atpkg_agents:$PATH\""),
                    "agents/ prepends"
                );
                assert!(
                ...
                assert_eq!(
                    ...
                    2,
                    "one idempotency guard per element"
                );
                assert!(
    ...
    #[test]
    fn fish_dialect_appends_bin_prepends_agents_and_avoids_posix_export() {
        let files = hook_files(Path::new("/p/bin"), Path::new("/p/agents"));
        ...
        assert!(
            fish.contains("set -gx PATH $__atpkg_agents $PATH"),
            "fish prepend for the agents dir"
        );
        assert!(fish.contains("set -gx ATPKG_AGENTS $__atpkg_agents"));
        assert_eq!(
            fish.matches("if not contains").count(),
            2,
            "fish idempotency, per element"
        );
