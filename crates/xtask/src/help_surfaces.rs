...
const SURFACES: &[SurfaceRow<'static>] = &[
    ...
    (
        ...
        "16c0afe8d32a66ef",
        "2026-09-12",
        "re-read on 2026-09-12 against the code that moved under it since the roster was minted, by the 2026-09-12 drift sweep (lane aterm-gui-2); no claim contradicted the moved code",
    ),
    (
        "crates/aterm-gui/src/control.rs",
        "f48165c575f385e9",
        "2026-09-12",
        "re-read on 2026-09-12 against the code that moved under it since the roster was minted, by the 2026-09-12 drift sweep (lane aterm-gui-1); no claim contradicted the moved code; re-hashed because its prose moved upstream after the row was minted",
    ),
    (
    ...
    (
        ...
        "ec55f57b8d6ec4ca",
        "2026-09-13",
        "cursor-home integration: changed prose read against merged fx, typed-turn/momentum and asynchronous privacy handlers; pending/baseline/coverage/interval claims corrected; 2026-09-13 (UTC; 2026-09-12 local) re-read of the paste-sweep additions to cmd_trail's doc (the licence= classes, program-row, the delivered-insert counters, which inputs stamp an insert and which stay dark) against aterm-effects cursor_glow.rs AdmissionRecord::line/LICENCE_*, log_licensed_as/log_decline/DECLINE_PROGRAM_ROW, note_insert_delivered/lay_insert/retract_insert/insert_tally and TrailStatus::line, app_render.rs tick_cursor_fx's Rainbow-Kitty-gated delivery prelude, app_input.rs input_paste's DeliveryTicket and the bare-Tab/⌃V insert_gesture_armed arm, lib.rs drop_file/deliver_paste, and control.rs's flagless, front-routed, background (cross_input/cross_raw_input), guarded (input_if_row_matches) and paste-bin (run_feed_bin_routed) routes for paste, turn, key, send and feed, fixing four slips in the text: paste-bin into the tab on screen reaches input_paste and stamps an insert (it had been listed as dark), the insert class and the inserts_* counters exist under Rainbow Kitty only, inserts_delivered also counts a bare Tab and ⌃V (not only pastes) and last_insert_cells is the 32-cell bound for an unpriceable insert, and `this verb's paste` named a paste form trail does not have; a guarded `key if=` and any background-routed input are now named as the dark cases; the 0.84 train's release-candidate merge (this machine's provenance/repair/TCC work over the peers' 2026-09-12 re-read at the tip) moved the bytes once more — both halves were read by their authors as recorded, the merged file is their union, and this row records the union's hash",
    ),
    (
    ...
    (
        ...
        "d8f5af6b89891d55",
        "2026-09-12",
        "re-read on 2026-09-12 against the code that moved under it since the roster was minted, by the 2026-09-12 drift sweep (lane aterm-gui-1); no claim contradicted the moved code",
    ),
    (
    ...
    (
        ...
        "1fad42a715d02aef",
        "2026-09-13",
        "re-read on 2026-09-12 against the code that moved under it since the roster was minted, by the 2026-09-12 drift sweep (lane small-2); no claim contradicted the moved code; re-hashed because its prose moved upstream after the row was minted; the 0.84 train's release-candidate merge (this machine's provenance/repair/TCC work over the peers' 2026-09-12 re-read at the tip) moved the bytes once more — both halves were read by their authors as recorded, the merged file is their union, and this row records the union's hash",
    ),
    (
        "crates/xtask/src/gate.rs",
        "ca0215e182b6ad14",
        "2026-09-12",
        "re-read on 2026-09-12 against the code that moved under it since the roster was minted, by the 2026-09-12 drift sweep (lane small-2); no claim contradicted the moved code",
    ),
    (
