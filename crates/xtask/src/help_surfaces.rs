...
const SURFACES: &[SurfaceRow<'static>] = &[
    ...
    (
        ...
        "9b2208c8d48d383a",
        "2026-09-14",
        "read crates/aterm-agent/src/lib.rs (DRIVE_HELP's await-turn timeout sentence, supervise's TIMEOUT sentence, watch's dedup and last-line paragraphs, and the rewritten --reconnect-s entry) against supervise/run.rs Session::call/unserved/ride_out (the outage-wide window from the first unserved request, one RECONNECT and one RECONNECTED per outage, 0.5 s doubling to 8 s across it, Rode::Spent → TIMEOUT, no such session not yet an answer only inside an outage, ERR exited always final), drive's fresh-only handed reset, moved_past's check read, the fallback press's Pressing::Lost handling and look's stray check, spent_turn/render_phase's `no screen` reason, drive_cli.rs main_entry (an Err exits 1) and await-turn's timed_out → 124, CtlReply::lost's TURNED_AWAY lines, and aterm-ctl's resolve_path (--sock, then $ATERM_CONTROL_SOCK, then self_instance_sock, then aterm.sock) and the `instances` verb's per-instance socket column (crates/aterm-ctl/src/lib.rs), by feat/handoff-survival on 2026-09-12 after the adversarial review of f6845c667; one slip fixed before this row (watch's dedup said the point is reprinted when the connection was lost, where the code resets on an outage's first answer, turned-away requests included — reworded to an outage came); the rest of DRIVE_HELP unchanged since the round-4 read of 2026-09-12; 2026-09-13 read of DRIVE_HELP's round-7 additions (shown by `xtask gate help-surfaces --diff`, matched at 718dda231) — the watch/report synopsis lines, watch's --report paragraph, the report entry and the two examples — against drive_cli.rs parse_sub (--since through Mark::parse, --max-rows > 0, --report) and its report arm (an Err exits 1), supervise/report.rs (newest_turn over `history 8`, find_start/is_user_row/user_text/paste_like/paste_fits with MARKER_CHARS 60 and PASTE_CHARS 200, offscreen_snapshot's since=/tail= max= screen=1 read, parse_offscreen's stderr header and its NoHeader fallback to `text --json`, join's back_at/pin skip of re-shown rows the read got, assess, gap_after's one-row recheck, finish's blank trim, Report::header/render), phase.rs transcript_end/is_done_row/is_parked_above_composer, run.rs reported/reported_event_line, and Turn::run (prompt types with send + key enter, so it leaves no ledger turn); two slips fixed before this row (main-screen said `the screen alone`, where the one offscreen read still joins the rows archived since the mark when the worker left the alternate screen — it now says what the reason means; complete=1's conditions left out the host keeping an archive, which no-archive refuses) and the reasons paragraph reflowed (one line had run to 109 columns); 2026-09-13 read of DRIVE_HELP's round-8 additions (shown by `xtask gate help-surfaces --diff`, matched at 547991d5b) — the supervise and watch synopsis `[--dismiss-surveys]`, phase's `survey 0` paragraph, await-turn's `exactly like phase`, supervise's withheld-fallback clause and survey sentence, watch's `EVENT survey` paragraph and the --dismiss-surveys entry — against drive_cli.rs parse_sub (--dismiss-surveys refused off watch and supervise) and phase_reply (phase and await-turn both render_phase_and_survey, 124 on a timed-out turn), phase.rs survey_open (the `●` question row in column 0 right over the options row, then only blank rows, right-aligned hints and `⎿  Tip:` rows down to the top rule, never without the composer frame), run.rs survey/hand_survey/dismiss_survey/typed_draft/survey_event_line and press_one_guarded's Withheld arm, StopAtReview::say (stderr) and Lines::say (stdout), and the server's input_if_row_matches (the check and the key under one terminal lock, `OK skipped` when no visible row matches; crates/aterm-gui/src/control_input.rs); three slips fixed before this row (the guard was said to let no copy of the survey on the screen through, where only a quoted copy — indented, or under `⎿` — is sure not to match; supervise's survey sentence said --dismiss-surveys dismisses it and says DISMISSED, where the line comes only once a fresh read shows it gone; a `0` found in the composer was put down to the survey leaving first alone, where the code backspaces one an open survey did not take as well); 2026-09-13 read (2026-09-14 UTC) of DRIVE_HELP's round-9 additions (shown by `xtask gate help-surfaces --diff`, matched at 20cd46a07) — the supervise and watch synopsis `[--context-warn PCT]`, phase's `context <n>%` paragraph, await-turn's `survey 0` and `context <n>%` clause (the rest of that paragraph reflowed, its words unchanged), supervise's stderr sentence, watch's `EVENT context`/`EVENT compacted` paragraph and the --context-warn entry — against drive_cli.rs parse_sub (--context-warn refused off watch and supervise, a value 0 to 100) and DEFAULT_CONTEXT_WARN, phase_reply (phase and await-turn both render_phase_and_survey, context after survey 0), phase.rs context_left (status_block's from to the top rule, is_against_right_edge, context_reading's two spellings), run.rs watch_context (the inclusive threshold, the frame-and-no-box rule for gone, the 30-point rise, the re-arm), await_turn_from's per-read call, drive's arming at each run's start, StopAtReview::say (stderr), Lines::say (stdout) and the tests supervise_watches_one_run and the_context_watch_is_inclusive_and_needs_the_frame; one slip fixed before this row (phase's paragraph put the indicator under the status row alone, where with no status row it is read under the last transcript row); 2026-09-14 read of DRIVE_HELP's round-10 changes (shown by `xtask gate help-surfaces --diff`, matched at cc86c5cfe) — the report entry's archive-reset and archive-gap clauses and its no-turn start — against aterm-gui handoff_carry.rs (export, tail_from, encode_within, decode and adopted_ledger: one sidecar carries the ledger and the archive tail, and a dropped or busy-locked ledger leaves the adopted one empty), aterm-core alt_archive.rs AltArchive::import (the carried origin kept; Refused when its indices are out of range or the archive is off) and carry_rows, spawn.rs's install right after restore_checkpoint, supervise/report.rs gather_report/ledger_turn/assess (archive-reset only when the mark's origin is not the read's, so a dropped carry opens at marker=user-row and only --since an older mark says archive-reset) and seamless_carry_tests' end-to-end resize variant (archive-gap, duplicates allowed, no row lost); one slip fixed before this row (the entry implied a failed carry reports archive-reset, where a ledger it could not carry leaves no turn and the start is the last `❯` row — the no-turn sentence now names that cause); 2026-09-14 read of DRIVE_HELP's round-11 additions (the supervise/watch synopsis `[--journal FILE]`, report's `[--final | --messages]` and its two view paragraphs, the whole `ledger` entry, and the `--journal FILE` entry) against drive_cli.rs parse_sub (--journal on watch/supervise/ledger only, --final/--messages on report only and never both, --format text|md|html and --out on ledger only, ledger's --since through supervise::parse_since where every other verb's is a Mark), ledger_verb (the worker from @sid else journal_sid's single sid, else an actionable error; the ledger dispatched before the host preflight; --out through write_private at 0o600 and the `wrote <path>` reply), clock_anchor (fleet_sessions for the pid, local_instances for the socket, its birth time as the process clock's zero) and tz_offset_s/parse_zone (`date +%z`, UTC when it cannot run), supervise/ledger.rs gather (whoami, `history`, one `offscreen tail=20000 max=20000 screen=1`, `@self inbox --peek --meta`, `@self timeline`, each miss a SOURCES row), reply_sizes (report's own join and the `❯` row found by the turn's first 60 characters or a fitting `[Pasted text …]` row, a turn whose row is nowhere named in the span that holds it), summary/items (the latency from each EVENT idle/question to the next turn's start, only with the clock placed; `~` on every unplaced time; a carried turn with no time), render_text/render_md/ledger_html::render_html (the four lanes, the inline style and script, the `default-src 'none'` policy), supervise/blocks.rs (the head rules NOTICE_HEADS/is_call_head/the next-row `⎿` rule/is_group_row, and view_rows: Final is the last Message block and the first Done after it, Messages every User and Message block without its `⎿` runs plus each Done row) and report.rs render_view (`view=`/`kept=` after `last=`, `rows=` unchanged, View::All byte-identical to render); measured for this row: the control socket's birth time is within 0.12 s of the fabric bus's own stamps on the same three messages on this machine, where `ps -o lstart=` was 5.3 s early; the --final clause was sharpened after the live read of 2026-09-14 showed blocks.rs is_group_row takes a group STILL RUNNING (`Running 3 shell commands · 9m 13s…`) for a tool row too; no claim contradicted; 2026-09-14 read again on the merge branch of every DRIVE_HELP block this round added: the `ledger` block against drive_cli.rs ledger_verb (the worker is `@sid` else journal_sid, and a journal naming two sids names none) and ledger.rs reply_rows, whose request is `offscreen tail=20000 max=20000 screen=1` from LEDGER_MAX_ROWS = 20_000 exactly as the help spells it, and against gather/render_ledger, which name a source they could not read and still return a Reply::text, so the exit stays 0; the `--journal` block against journal.rs JournalRecord::of_line (the six kinds the loops write - event, approved, dismissed, reconnect, timeout, exit - and the eight phase words, every field taken from the line) and Journal::open (create + append, 0600 only on creation, one warning); `report --final|--messages` against report.rs render_view (` view=<name> kept=<n>` appended after the header's own `last=`, `rows=` still the whole report, View::All returning render() unchanged) and blocks.rs view_rows/head_kind/is_tool_head; --format, --out, --since and --journal ownership against parse_sub's per-verb refusals; no claim contradicted, and one slip fixed OUTSIDE the roster in blocks.rs's module doc, which named five of NOTICE_HEADS' eight notices and gave `searched memories` as a whole collapsed-group row where is_group_row requires the first clause capitalised; 2026-09-14 (feat/round-14-mail, round 14 D1/D2/D4) read of the --mail and task prose — DRIVE_HELP's synopsis lines, the watch, task and --mail entries and the journal's mail kind and report field (lib.rs); DRIVE_PAGE's watch, task, --mail and --journal entries (manual.rs); run.rs's module doc, SuperviseOpts::mail, Fold, MailIn, NoLane, Sink, the Review, StopAtReview, Lines and folded_event_line docs, supervise_mail, supervise_with, run_loop, watch_mail, watch_with, look's fold gate, hold_for_report, event_line_as, render_result_mail and the six mail test docs; the two skills' loop, watch, task and ledger paragraphs — against mail.rs Lane::serve (an inbox 1 --peek --meta baseline; await inbox since=<newest> kinds=<all nine> timeout <step>, re-armed on OK timeout, the kinds list dropped on ERR usage; inbox since=<id> --peek --meta, then inbox get <id> for the watched worker's report only; the MAIL line per row through the loop's sink; lane_off said once on any other ERR or a lapsed reconnect window), Lane::call's retry of a lost reply (pause doubling to pause_max, within reconnect), task() (post to=@sid kind=task [dl=<ms>] <text> from --inbox or @self, the offset from the OK's off=, one text --json read and `turn idle=600 timeout=2500 Inbox: task @<off>` only on Phase::Idle with submitted=1 as nudged, the inbox 1 baseline read BEFORE the post, await inbox … kinds=answer,report,ack re-armed on rows without re=<off>, the TIMEOUT line and EXIT_TIMEOUT), run.rs hold_for_report (pending drained; a delivery within the window folds at once, else recv_timeout to min(now + grace, deadline); Disconnected gives None so the line is as without the flag), look's Phase::Idle gate with the brief skipped only on Fold::Report, run_loop's thread::scope join (the lane returns within one mail_step) and drive_cli.rs parse_sub (--mail, --report-window and --idle-grace watch's and supervise's only; --inbox needs a leading @; --deadline, --wait and --no-nudge task's only), mail_needs_sid and task_opts (--wait bounded by --deadline, else --timeout); measured on the live worker s-1e918c4662a1b7b8bd43 (busy for the whole 25 s budget: TIMEOUT, the process ending 40 s in as the lane's parked wait was joined within its 20 s step, no MAIL lane off) and by the mock tests; no claim contradicted; 2026-09-15 read of DRIVE_HELP's --mail and task entries as the round-14 reviews (wake-economy, report-hook-safety) changed them — the one screen read per 20 s hold step, a superseded point said as idle-no-report with what followed (a footer tick held on), a report attributed to its turn (after the worker was read busy for it, or after the point; never one from before the last point handed over or between it and the next busy read; --report-window only for one from before the turn was seen to begin), the lane's parked wait cut short (aterm-ctl signalled) so supervise --mail returns at once and watch --mail ends with its last line, the resync from the bus offset after a self-update, and task --wait re-armed past the host's 600 s clamp — against supervise/run.rs hold_for_report (hold_step slices, review_key compare, Held::carried), MailIn::is_this_turns (floor, busy_at, handed, window), await_turn_from's busy_at stamp, look's floor reset on reported() points, run_loop's Ctl::interrupter call, lib.rs CtlClient::cutter (SIGTERM to the pid under the lock, the cut flag checked after spawn), supervise/mail.rs Lane::resync (inbox --peek --meta whole, rows above the last off, since= the successor's newest id) and task's re-arm (OK timeout → continue while ≥ 1 ms is left), by feat/round-14-mail on 2026-09-15; no claim contradicted; pinned by run.rs's a_question_turns_report_is_not_the_next_turns, a_second_report_of_a_turn_is_not_the_next_turns, a_mid_turn_report_older_than_the_window_is_still_its_turns, a_prompt_after_an_idle_is_seen_within_a_hold_step, supervise_mails_result_does_not_wait_for_the_lanes_parked_step and mail.rs's a_wait_re_arms_on_the_hosts_own_timeout_while_time_is_left, the_lane_hears_the_successors_rows_after_a_handoff; 2026-09-14 (2026-09-15 UTC) read, before the merge, of every prose line `xtask gate help-surfaces --diff` shows moved since main's recorded read (matched at 6318eb4f6): DRIVE_HELP's synopsis lines, the watch entry's --mail sentence, the whole task entry and the whole --mail entry — against supervise/mail.rs (Lane::serve: the `inbox 1 --peek --meta` baseline, `await inbox since=<id> kinds=<nine> timeout <step>` re-armed on OK timeout, the kinds list dropped on a usage error, the listing since=<id> and `inbox get` for the watched worker's report only; Lane::resync after a lost request, from the bus offset; task(): the baseline read before the post, `post to=@sid kind=task [dl=<ms>] <text>`, `task not posted: <ERR>` and `task not landed` for a reply without off=, one `text --json` read and `turn idle=600 timeout=2500 Inbox: task @<off>` on Phase::Idle only, submitted=1 as nudged, `await inbox … kinds=answer,report,ack` re-armed on the host's timeout while at least 1 ms is left, the TIMEOUT line and EXIT_TIMEOUT), supervise/run.rs (MailIn::is_this_turns: floor, busy_at, handed, window; hold_for_report: pending drained, the newest of this turn's folds, else recv_timeout in hold_step slices with one screen read per slice, an equal review_key holds on, else NoReport with the screen carried; look: the carried screen is the next turn unless busy; run_loop: the stop flag then the interrupter, thread::scope join; render_result_mail; folded_event_line), lib.rs CtlClient::cutter (SIGTERM to the registered pid, the cut flag checked after the spawn), drive_cli.rs parse_sub/mail_needs_sid/task_opts and aterm-gui fabric.rs cmd_post (WAIT_DEFAULT_MS = 30 s, `OK <id> off=<n>` on a landing, `ERR timeout id=<n>` past the wait); ONE claim contradicted and fixed in this commit: the task entry said a worker with round 12's wake hooks reads a task on its next Stop, where aterm-link hook.rs may_wake/accepted wakes only for a human or a sender whose OWNER (`s-…`) the hook's --accept-from lists — measured on the owner's live worker, whose hooks list the node id and whose inbox holds three of the manager's tasks, kind=task, none woken for — so the entry now says the manager's sid must be in --accept-from; no other claim contradicted",
    ),
    (
        "crates/aterm-cli/src/lib.rs",
        "d741990fcd6a334c",
        "2026-09-16",
        "re-read on 2026-09-12 against the code that moved under it since the roster was minted, by the 2026-09-12 drift sweep (lane small-1); no claim contradicted the moved code; 2026-09-14 read of the one verb added since (`Verb::Fabric`: its doc, `usage()` `aterm fabric [<cmd>]`, the `argv0_alias` note that `fabric` never was a sibling binary, and the four-line blurb) against crates/aterm/src/main.rs's exhaustive verb match (`Verb::Fabric` routes to aterm_link::fabric::main, above the mode fork like every verb) and crates/aterm-link/src/fabric.rs (parse_args: no argument is `status`, `tail` is the other subcommand; gather/render_text: config, broker, every local instance's bridge, every session's inbox numbers, the last TRAFFIC_ROWS = 10 bus records and the warnings; resolve_command reads `[fabric] command`), by feat/round-11-fabric-ledger; the blurb's 26-column synopsis fits the VERB_BLURB_COLUMN rule verb_help_blocks_align_to_one_column pins; no claim contradicted; 2026-09-14 read again on the merge branch: Verb::Fabric's doc, `aterm fabric [<cmd>]`, the argv0_alias comment (`fabric` never was a sibling binary, and the arm answers None) and the four blurb lines against crates/aterm/src/main.rs's Verb::Fabric arm, which calls aterm_link::fabric::main on the forwarded argv above the mode fork, and against fabric.rs (TRAFFIC_ROWS = 10 under `the last 10 bus records`, resolve_command under `it reads the [fabric] command`, parse_args under `status | tail`); no claim contradicted; 2026-09-14 read of the two lines added to Verb::Fabric's doc and blurb (`on|off|doctor`: `on` turns it on in one command and proves it, `off` turns it off, `doctor` names the fix for each warning) against crates/aterm-link/src/fabric.rs parse_args (the three verbs and their flags) and crates/aterm-link/src/enable.rs on (eleven steps ending in the self-post proof and the status print), off (the job, the `[fabric]` table and the rendezvous file removed; node id, secret and cap kept) and doctor (fix_for under every warning), by feat/round-13-fabric-on; the six-line blurb still renders under VERB_BLURB_COLUMN (verb_help_blocks_align_to_one_column); no claim contradicted; 2026-09-14 read of `Verb::Link`'s row against the audit's finding that `argv0_alias()` had no consumer: a new test in this crate, every_argv0_alias_is_bundled_and_routed, reads crates/aterm-release/src/bundle.rs's symlink loop and crates/aterm/src/main.rs's alias_route and asserts every alias the roster yields is in both — the pin that would have caught `aterm-link` missing from the bundler; no help text changed, only the roster gained a reader; 2026-09-14 merge read of main over 28508563a: the merged prose is exactly round 13's `on|off|doctor` blurb and overview lines (checked against crates/aterm-link/src/fabric.rs parse_args' on/off/doctor arms and enable.rs on/off/doctor) plus main's every_argv0_alias_is_bundled_and_routed doc and assertion strings, checked against crates/aterm-release/src/bundle.rs's `for alias in [...]` symlink loop (each alias a quoted string, symlinked to `aterm`), crates/aterm/src/main.rs alias_route's `aterm-ctl` / `atpkg` / `aterm-fleet` / `aterm-drive` / `aterm-link` => AliasRoute arms, and crates/aterm-release/Cargo.toml's dependency table, which names no aterm-cli; no claim contradicted; 2026-09-14 read of the prose 65bb10c42 added (the `trusts:` sentence of `version_text`'s doc: `trusts: master=<sha256> channel=<sha256>`, the paper master's and the channel key's compiled-in fingerprints, `empty` an unarmed tier, printed last even for `None`, and the `trusts: master=` / ` channel=` / `{anchors}` strings of the version test) against trust_anchors_line (formats `trusts: master={} channel={}` from aterm_update::compiled_master_pin_sha256 and compiled_update_pin_sha256), crates/aterm-update/src/lib.rs's fingerprint_state (SHA-256 of the raw 32 bytes of pins::PAPER_MASTER_PUBKEYS[0] and of PINNED_UPDATE_PUBKEY, the channel signing key, via update_pubkey_sha256 — `empty` on an empty pin, `invalid` on a malformed one) and version_text itself (identity line, the which-copy lines, then the anchors line last, which version_text_names_the_running_copy_and_any_other pins for `None` and for a copy); no slip found; 2026-09-15 merge read: version_text now emits a published nonzero build immediately after its identity line and before copy diagnostics, with trust anchors last. Read against set_running_build first-publication semantics, the one-binary mode-free dispatcher and verify::probe_bundle_starts. Unpublished builds omit the line; the candidate plist remains the authoritative build check. The version regression verifies the order and retained first value.; 2026-09-15: the [machine] explain-config paragraph read against app_config.rs MachineConfig resolvers and atpkg config.rs (same table, same defaults); no restart/relaunch wording (the privacy test's whole-report scan); 2026-09-15 read at the merge of feat/round-15-receipts over main of the prose f3a5467ed moved with no row (shown by `xtask gate help-surfaces --diff`, matched at 0fc02ea5d): help_text's `{HELP_TITLE}{}\\n{HELP_HEAD}` (the title line, then aterm_types::identity::ORIGIN_LINE `by Andrew Yates · ALab · alab.systems`, then HELP_HEAD from its leading blank line), version_text's `aterm {}\\n{}\\n` (the identity line install.sh greps `^aterm ` stays first, the origin line second, then build: and which copy runs) and the version test's doc and its `by Andrew Yates`/`alab.systems` needles — against help_text, version_text and identity.rs ORIGIN_LINE (assembled from AUTHOR, COMPANY and SITE, pinned by its own test); no claim contradicted; landing of the erase pending-wrap fix, over f3a5467ed: the --help title split into HELP_TITLE with the origin line printed under it, the --version origin line under the identity line, and the reworded version_text test doc, read against help_text and version_text, parse_args and the launcher mode-free route, aterm_types::identity::ORIGIN_LINE and its parts test, the tools/install.sh ^aterm grep, the cutter validate_named_cli_app_version origin admission, the updater start probe build match, and the test exact-order assertion; no claim contradicted the code; 2026-09-15 merge read of main (round 15) over origin/main 58bb2fc63: the merged prose is round 15's read change plus main's, and it hashes to this row's recorded value, so no prose moved at the merge 2026-09-15: re-read the `[privacy]` block of explain-config against crates/aterm-gui/src/app_config.rs's privacy_warmup_folders (which defaults to aterm_containment::Folder::ALL) and aterm_containment::consent::Folder after the roster gained `app-data` \u{2014} the warmup_folders paragraph now names what that gesture asks for (the folders, and app-data: every other app's own data); no other claim moved. 2026-09-16 audit-3 read: MACHINE_CONFIG_PARAGRAPH re-read against the code it describes — spotlight_noindex now says the scan reaches what it reaches and names BOTH pointers (a `target` symlink in a git checkout, a `[build] target-dir` line in .cargo/config.toml elsewhere, per noindex::pointer_for) and the SKIP_DIRS folders it never walks; the refusals now include an aterm.toml that does not parse (config::parse_machine's unreadable flag, cli::machine_refusal); and the undo carries both halves — the `defaults delete` pair AND `universal_control = \"leave\"`, without which the next pass disables it again (machine::doctor_line's ok arm).",
    ),
    (
        "crates/aterm-cli/src/manual.rs",
        "c12ea8e7396b60b9",
        "2026-09-16",
        "read crates/aterm-cli/src/manual.rs (DRIVE_PAGE's await-turn and supervise timeout clauses, watch's dedup and EXIT list, and the rewritten --reconnect-s entry) against aterm-agent supervise/run.rs Session::call/unserved/ride_out (one outage window, lines once per outage, Rode::Spent → TIMEOUT, no such session not yet an answer inside an outage), drive's fresh-only handed reset, moved_past's check read, spent_turn, CtlReply::lost's TURNED_AWAY lines, drive_cli.rs main_entry's exit 1, and aterm-ctl's resolve_path order (crates/aterm-ctl/src/lib.rs), by feat/handoff-survival on 2026-09-12 after the adversarial review of f6845c667; no slip found in this pass; every other page unchanged since the 2026-09-11 read; AND read crates/aterm-cli/src/manual.rs (DRIVE_PAGE's phase, await-turn, supervise and watch entries) against aterm-agent supervise/phase.rs worker_phase/busy_signal/limit_notice, supervise/run.rs await_turn_from/drive/review_key/watch and drive_cli.rs main_entry/watch_exit_line, by aterm-supervise-round-4 on 2026-09-12 after the round-4 review; one slip (the dedup not naming the reset on an approval) fixed before this row; every other page unchanged since the 2026-09-11 read; 2026-09-12 re-read of the one sentence added since (a launch pass that finds another aterm's install in flight waits for it, then runs, and never reports it as a failure) against spawn_pkg_update_check's --wait-lock / ATPKG_WAIT_LOCK_SECS path, the Wake::PkgLockWaiting info row that is never the failure bar, and contention_is_busy_never_refused_regression_2026_09_10; the merged file is exactly those two read changes together (the drive page from feat/handoff-survival, the launch-wait sentence from main), recorded at the merge; 2026-09-13 read of DRIVE_PAGE's round-7 additions (shown by `xtask gate help-surfaces --diff`, matched at e2ae167f2) — watch's [--report] and its --report sentence, and the report entry — against the code crates/aterm-agent/src/lib.rs's row names (drive_cli.rs parse_sub and the report arm, supervise/report.rs gather_report/newest_turn/find_start/join/assess/Report::header, phase.rs transcript_end, run.rs reported_event_line); one slip fixed before this row (`main-screen or no-archive (the screen alone)`: main-screen still carries the rows archived since the mark when the worker left the alternate screen — it now says the main screen's scrollback is not read) and the header synopsis reflowed; 2026-09-13 merge of main into feat/round-7-offscreen: main re-read this page on 2026-09-13 (`aterm help fabric` carried three false fabric claims: post refuses with no bridge, and the halt shape) and fixed it; `gate help-surfaces --diff` at the merge shows the merged prose is exactly round 7's read DRIVE_PAGE additions plus main's three fabric corrections (a post that waits is refused no-bridge=1, others queue; ERR halted reason= origin=), spot-checked against fabric.rs fabric_wait_refusal; recorded at the merge; 2026-09-13 read of DRIVE_PAGE's round-8 additions (shown by `xtask gate help-surfaces --diff`, matched at bd8f70ca9) — phase's `survey 0` sentence, await-turn's `survey 0` clause, supervise's `[--dismiss-surveys]`, withheld-fallback clause and survey sentence, watch's `EVENT survey` sentence and the --dismiss-surveys entry — against the code crates/aterm-agent/src/lib.rs's round-8 read names (drive_cli.rs parse_sub and phase_reply, phase.rs survey_open, run.rs survey/dismiss_survey/typed_draft/survey_event_line and press_one_guarded's Withheld arm, the server's input_if_row_matches); three slips fixed before this row (a copy in the transcript narrowed to a quoted one; the fallback was said never to press while the survey is open, where it checks the confirming read — reworded, the box named as yours; the --dismiss-surveys entry implied a DISMISSED line and a note for a skipped `0`, which gets neither) and await-turn's clause reflowed; 2026-09-13 read (2026-09-14 UTC) of DRIVE_PAGE's round-9 additions (shown by `xtask gate help-surfaces --diff`, matched at 20cd46a07) — phase's `context <n>%` sentence, await-turn's clause, supervise's and watch's `[--context-warn PCT]` and their EVENT context/compacted sentences, and the --context-warn entry — against the code crates/aterm-agent/src/lib.rs's round-9 read names (drive_cli.rs parse_sub, DEFAULT_CONTEXT_WARN and phase_reply, phase.rs context_left, run.rs watch_context/await_turn_from/drive and the two Review says); one slip fixed before this row (phase's sentence called `context <n>%` a last line right after `survey 0`'s last line — it now says it comes after any `survey 0`) and await-turn's clause reflowed; 2026-09-14 read of DRIVE_PAGE's round-10 report clauses (shown by `xtask gate help-surfaces --diff`, matched at cc86c5cfe) — archive-gap's resize as a self-update takes over, and archive-reset's carried and not-carried cases with the empty `history` — against the code crates/aterm-agent/src/lib.rs's round-10 read names (handoff_carry.rs export/tail_from/CARRY_ARCHIVE_BYTES/adopted_ledger, alt_archive.rs import/carry_rows, report.rs gather_report/assess, seamless_carry_tests' end-to-end, resize and dropped-carry tests); one slip fixed before this row (the report was said to be whole across a self-update that carries the archive, where the carry keeps at most the newest 1 MiB of rows — now whole when the rows since the turn fit what it carries) and the reasons' tail reflowed; 2026-09-13 read of the fabric-page prose moved since that read (shown by `xtask gate help-surfaces --diff`, matched at cc86c5cfe91c) — FABRIC_PAGE's new A WAIT THAT DOES NOT LAND block, the rewritten `connected` and `disconnected` rows of IS IT ON HERE?, the `aterm-link` to `aterm link` respellings (hook install, the mirror synopsis, the fabric_page doc) and the test needles that pin them — against aterm-gui fabric.rs cmd_post (its option grammar is to/kind/re/dl/via/len/--wait and PostRow keeps no caller-chosen key, so `post` really carries no idempotency key and the broker's (producer_id, producer_seq) dedup cannot collapse a re-post), fabric_wait_refusal over bridge_reachable (queued=1 while a supervisor is armed or a bridge has ever attached, no-bridge=1 otherwise) and the wait loop's deadline arm (`ERR timeout id=<n>`), cmd_outbox's PEEK with control.rs cmd_fabric_attach → fabric_launch::arm/note_bridge_supervised/supervise (a post refused no-bridge=1 is still in `posts`, the attach arms a supervisor that relaunches for ever, and the next bridge drains that same queue — so `aterm ctl fabric attach <command...>` does drain the same outbox, and `fabric` is a Meta/Owner verb so no selector is needed), control.rs serve_bridge's bridge_attached (connected is stored when the bridge's INHERITED fd starts being served, before any broker contact) and bridge_lost (disconnected plus the fabric-lost hold only when a bridge fd closes) with aterm-link bridge.rs Bridge::run (an unreachable broker is retried RECONNECT_MIN..RECONNECT_MAX for ever and the bridge exits only when aterm closes, so a bridge aimed at a socket that does not exist reports connected and killing the BROKER cannot move the state off connected — both claims true), and the respellings against aterm/src/main.rs Verb::Link → aterm_link::cli::dispatch's hook/mirror arms, hook.rs install_claude (four hooks — SessionStart, UserPromptSubmit, PreToolUse, Stop — into .claude/settings.json) and mirror.rs (`<root> --sock <path>`, the four inbox/outbox/sent/.cursor files, POLL_DEFAULT 250 ms), with aterm-release bundle.rs's alias list showing why the verb spelling is the safe one (aterm-link was only just added to the shipped argv0 symlinks); ONE CONTRADICTION, left for the orchestrator: the block's header says a wait that does not land has THREE answers and that ALL THREE MEAN QUEUED, where cmd_post has a FOURTH — a post the bridge retires with `outbox sent <sid> <id> off=- reason=<why>` (bridge.rs Route::reason gives `unroutable` and `ambiguous`, the endpoint's DEAD_DEFAULT is `undeliverable`) wakes the parked wait through retire_post's notify_all and answers `ERR <reason> id=<n>`, and that is the one answer that does NOT mean queued: the row is dead, cmd_outbox filters `!p.dead` so no bridge ever drains it again, and an agent applying the page's rule would report a message nothing will publish as queued. THE CONTRADICTION THAT READ FOUND WAS FIXED IN THIS COMMIT, NOT RECORDED AROUND: the block claimed a wait that does not land has THREE answers and that all three mean queued, and `cmd_post`'s wait loop has a FOURTH — a row the bridge RETIRED (`outbox sent <sid> <id> off=- reason=<why>`; bridge.rs Route::reason gives `unroutable`/`ambiguous`, the endpoint default is `undeliverable`) releases the wait with `ERR <reason> id=<n>`, and `cmd_outbox` filters `!p.dead` so nothing drains it again — the one answer that is NOT queued, which an agent obeying the page would have reported as queued. The page now says FOUR ANSWERS, AND THREE OF THEM MEAN QUEUED, names the fourth and what to do about it, and the test's needle list and its `THREE OF THEM MEAN QUEUED` assertion pin it; the hash recorded here is the CORRECTED page; 2026-09-14 CROSS-CRATE read of the WHOLE manual, not only DRIVE_PAGE — the limitation the drift sweep records is that a per-crate sweep never reaches a surface describing ANOTHER crate's code, and every page in this file does. 22 confirmed findings fixed, each read against the code the verifier cited: the agents page's primer shape (aterm-primer block_with writes PRIMER_BODY + RUST_NOTE + FABRIC_NOTE — three `##` sections, about forty lines, and only PRIMER_BODY self-gates on aterm; RUST_NOTE is headed \"true in ANY terminal\" and gates on $ATPKG_BIN, FABRIC_NOTE on `fabric=`); the atpkg page's \"CLI-only use never auto-updates\", said twice (crates/aterm/src/main.rs's session lane: session_lane_is_interactive + pkg_check::session_pass_due spawn a detached `aterm pkg update` once per interval — only a piped/ATERM_SESSION_MODEL launch and --headless provision nothing), its \"no vendor-fetched member is published\" (stub.rs AGENT_PROGRAMS: `claude` and `codex` are default-set members fetched from vendor.rs's VENDOR_HOSTS; the OS-installed half of the claim is true and kept), its rc-block list (hooks.rs's consent fence + protected.rs: an rc resolving under a macOS-guarded root is skipped since the 2026-09-12 TCC audit) and its HALF revert (machine::UNIVERSAL_CONTROL_REVERT deletes BOTH keys — the page printed one, and a new derived test checks the page against each command the const joins); the reroute page's \"a SIGNPOST row refuses\" (reroute.rs `run` + signpost_action: announce then exec UPSTREAM, refusing only under ATERM_REROUTE_STRICT — the page already said so twice below), the `lean` row's SourceVerb (`lean f.lean` runs `clean check f.lean`), the third line signpost_closing emits (the `aterm help rust` measuring line, absent from the reproduction) and EXPLICIT_LANE_VERBS (`cargo trust …` / `cargo --unverified …` exec_branded the BRANDED tool, so \"nothing is substituted\" was true of a bare cargo only); the config page's key count (159, measured off aterm-gui's STARTER_CONFIG and now pinned there by starter_config_key_count_matches_the_manual, which names this line) and explain-config's coverage (lib.rs PRIVACY_CONFIG_PARAGRAPH documents aterm.toml's nine `[privacy]` keys, so \"not the contents of the file above\" was wrong); the ship page's bare `provision` (apple.rs apple_identity_check and ensure_notary run with may_change — it ACQUIRES, asking before a permanent Developer ID slot and prompting for the notary password, so it is not \"the same audit\"); the drive page's busy zones (supervise/phase.rs Busy::hard(Zone::Hint, \"Still working\") is a THIRD zone between the status row and the footer, and Zone::Screen is what a frameless read names); the trust-backends page's \"installed on first launch\" (atpkg cli.rs seed_serviceable + current_triple: the rustc coherence tuple publishes aarch64-apple-darwin ONLY and is skipped whole on the five other client triples) and trust-cg's identity (stub.rs: the Trust compiler's CODEGEN member, not a certificate generator); rust_page's own verdict, which was a CODE fix rather than prose — it asked `tools.targo.is_file()` where discovery answers `Toolchain::have_targo()` (aterm-verify toolchain.rs: refused.is_none() && is_executable_file), so a REFUSED toolchain every stage was about to fail closed on printed `(wins)` on the page whose doc claims it calls the real discovery code; the introspection page's \"every window-mode session exposes a control socket\" (control_auth::resolve_socket_plan: --no-control-sock / ATERM_NO_CONTROL_SOCK=1 / ATERM_CONTROL_SOCK=0|off resolve Disabled) and its \"whole fleet on ONE fd\", said twice (control.rs proxy_forward_plan returns None on a comma and an unresolvable selector answers `ERR no such session`, so a comma list is instance-local; fleet_cli.rs stream_targets is the fleet-wide path); and the agent brief's \"no git hooks and no CI\" (.githooks/pre-push is tracked and aterm-verify's pin_hooks sets core.hooksPath on every run — the hook is ADVISORY, one printf and exit 0, and there is still no CI). Two candidates were REFUTED by their verifiers and deliberately left alone: a DRIVE_PAGE quote read as a claim it does not make, and a reviewer's \"no resource limits on Windows\" premise that is false at HEAD. The agent brief stays inside its own budget (89 lines / 6,333 bytes against 90 / 7,000), and the atpkg and introspection paragraphs touched here were re-wrapped. Rebased onto main's 90 further commits AFTER that read: `gate help-surfaces --diff` at the rebase shows the moved prose is exactly these 22 fixes and nothing of main's own reads was displaced, so main's method text above stands unedited and this note is appended to it; 2026-09-14 push-gate read (grep guard B13a): the atpkg page's rc-skip sentence typed the protected folders as home-relative path literals, which B13a refuses in aterm-cli source; reworded to name them as folders and re-read against hooks.rs's consent fence (canonicalize the rc, under_protected_root, continue: the rc stays unwired) and protected.rs's HOME_FOLDERS (the six home folders), LIBRARY_DOMAINS (Mobile Documents is iCloud Drive, CloudStorage is every third-party sync provider) and ABSOLUTE_ROOTS (network and removable volumes); every claim holds; 2026-09-14 merge read of feat/round-10-carry over main: the merged prose is exactly DRIVE_PAGE's round-10 report clauses (archive-gap's resize as a self-update takes over, archive-reset's carried and not-carried cases with the empty `history`; `xtask gate help-surfaces --diff` against main's read, matched at 9bf89d347, shows that one hunk and nothing else) plus main's reads recorded above (the fabric page's FOUR ANSWERS block and its `connected`/`disconnected` rows, the `aterm link` respellings, the 22 whole-manual fixes and the guarded-folders rewording; `--diff` against round 10's read, matched at a4a023d4a, shows those 29 hunks and nothing else), checked against the merged code: round 10's anchors (handoff_carry.rs CARRY_ARCHIVE_BYTES = 1 MiB, TAIL_TURNS = 8 and adopted_ledger, alt_archive.rs carry_reach and import, supervise/report.rs gather_report/assess) are round 10's own, main touching none of those files; main's (fabric.rs cmd_post's wait loop — the landing, a dead row's `ERR <reason> id=` from the bridge's reason or DEAD_DEFAULT `undeliverable`, fabric_wait_refusal, `ERR timeout id=` — and the `!p.dead` outbox filter; aterm-primer block_with's PRIMER_BODY + RUST_NOTE + FABRIC_NOTE; aterm main.rs session_lane_is_interactive; atpkg stub.rs AGENT_PROGRAMS, machine.rs UNIVERSAL_CONTROL_REVERT's two deletes, reroute.rs STRICT_ENV, EXPLICIT_LANE_VERBS, the `.lean` SourceVerb and signpost_closing's measuring line; supervise/phase.rs busy_signal's Zone::Hint `Still working`; control_auth.rs resolve_socket_plan's Disabled arm and control.rs proxy_forward_plan's comma refusal; .githooks/pre-push's advisory exit) are main's, round 10 having added only re-exports to control.rs and control_auth.rs; no claim contradicted, no prose changed at the merge; 2026-09-14 read of the two additions since (the overview's `Verb::Fabric` line and FABRIC_PAGE's IS IT ON HERE? entry for `aterm fabric`) against crates/aterm-link/src/fabric.rs: resolve_command (env first, then `[fabric] command`, no arguments), probe_broker (connect_bounded, `hello`, every cap attached, the `Fetch max=0` head query — the `(connect, attach, head query)` the entry names), gather's instance walk over aterm_ctl::local_instances (the bridge of every instance on this machine), read_session/fill_inbox (hold and the inbox numbers), last_records with TRAFFIC_ROWS = 10 and traffic_of (metadata only — the text is only printed by tail_line with `--bodies`), warnings(), status_main's exit (exit_of: 0 without a warning, 1 with one; 2 from Off) and tail (subscribe from the head or `--from`), by feat/round-11-fabric-ledger; every line stays within the 92-column bound fabric_topic_turns_it_on_with_the_shipped_binary pins; no claim contradicted; 2026-09-14 read again on the merge branch of the FABRIC_PAGE block this round added: the `aterm fabric` entry under IS IT ON HERE? against fabric.rs (no argument; probe_broker's connect + hello + attach + head query behind `reached for real`; gather's BRIDGES/SESSIONS/TRAFFIC sections; TRAFFIC_ROWS = 10; exit_of and the Off arms behind `0 healthy, 1 warned, 2 off`; tail's --bodies) and the overview line against aterm-cli's Verb::Fabric; no claim contradicted; 2026-09-14 read of DRIVE_PAGE's round-11 additions (the supervise/watch synopsis `[--journal FILE]`, report's `[--final | --messages]` clause, the `ledger` entry and the `--journal FILE` entry) against the same handlers the aterm-agent lib.rs row names — drive_cli.rs parse_sub/ledger_verb/clock_anchor, supervise/ledger.rs gather/reply_sizes/summary/items and its three renderers, supervise/blocks.rs view_rows and report.rs render_view — and against DRIVE_HELP itself, so the page and the help say the same thing in fewer words; no claim contradicted; 2026-09-14 read again on the merge branch of the three DRIVE_PAGE blocks this round adds: `ledger`'s and `--journal`'s entries against the same handlers the crates/aterm-agent/src/lib.rs row names (drive_cli.rs ledger_verb and parse_sub, supervise/ledger.rs gather/render_ledger, supervise/journal.rs), and report's `--final | --messages` clause against supervise/blocks.rs view_rows (the last Message block through the first Done row after it; every User and Message block plus the done rows, with without_gutter dropping the `⎿` rows and the output hanging under them) and report.rs render_view; no claim contradicted; 2026-09-14 round-12: FABRIC_PAGE's BEING WOKEN Claude Code row rewritten and read against aterm-link hook.rs — install_claude (--merge into an existing file with a <file>.bak-<unix> copy first; a bare install writes a new file and refuses an existing one; every generated command executed with --check and the install refused with exit 2, nothing written, when one does not answer ok), check (`ok session=<sid> sock=<path>`, resolved by aterm_ctl::resolve_sock_for: $ATERM_CONTROL_SOCK, then the rendezvous graph entry of $ATERM_PARENT_SESSION_ID, then the latest alias) and open (a hook that cannot reach aterm exits 0 with the reason on stderr; only pre_tool_use under hold=1 and stop with accepted mail exit 2); the live-load claim is the vendor's documented hooks behaviour, measured on 2026-09-14 when a failing hook blocked a running session; nothing else on the page moved; 2026-09-14 read of the two additions since (the overview's `Verb::Fabric` line and FABRIC_PAGE's IS IT ON HERE? entry for `aterm fabric`) against crates/aterm-link/src/fabric.rs: resolve_command (env first, then `[fabric] command`, no arguments), probe_broker (connect_bounded, `hello`, every cap attached, the `Fetch max=0` head query — the `(connect, attach, head query)` the entry names), gather's instance walk over aterm_ctl::local_instances (the bridge of every instance on this machine), read_session/fill_inbox (hold and the inbox numbers), last_records with TRAFFIC_ROWS = 10 and traffic_of (metadata only — the text is only printed by tail_line with `--bodies`), warnings(), status_main's exit (exit_of: 0 without a warning, 1 with one; 2 from Off) and tail (subscribe from the head or `--from`), by feat/round-11-fabric-ledger; every line stays within the 92-column bound fabric_topic_turns_it_on_with_the_shipped_binary pins; no claim contradicted; 2026-09-14 read again on the merge branch of the FABRIC_PAGE block this round added: the `aterm fabric` entry under IS IT ON HERE? against fabric.rs (no argument; probe_broker's connect + hello + attach + head query behind `reached for real`; gather's BRIDGES/SESSIONS/TRAFFIC sections; TRAFFIC_ROWS = 10; exit_of and the Off arms behind `0 healthy, 1 warned, 2 off`; tail's --bodies) and the overview line against aterm-cli's Verb::Fabric; no claim contradicted; 2026-09-14 read of DRIVE_PAGE's round-11 additions (the supervise/watch synopsis `[--journal FILE]`, report's `[--final | --messages]` clause, the `ledger` entry and the `--journal FILE` entry) against the same handlers the aterm-agent lib.rs row names — drive_cli.rs parse_sub/ledger_verb/clock_anchor, supervise/ledger.rs gather/reply_sizes/summary/items and its three renderers, supervise/blocks.rs view_rows and report.rs render_view — and against DRIVE_HELP itself, so the page and the help say the same thing in fewer words; no claim contradicted; 2026-09-14 read again on the merge branch of the three DRIVE_PAGE blocks this round adds: `ledger`'s and `--journal`'s entries against the same handlers the crates/aterm-agent/src/lib.rs row names (drive_cli.rs ledger_verb and parse_sub, supervise/ledger.rs gather/render_ledger, supervise/journal.rs), and report's `--final | --messages` clause against supervise/blocks.rs view_rows (the last Message block through the first Done row after it; every User and Message block plus the done rows, with without_gutter dropping the `⎿` rows and the output hanging under them) and report.rs render_view; no claim contradicted; 2026-09-14 merge read of feat/round-12-hooks over main: the merged prose is exactly this branch's rewritten BEING WOKEN Claude Code row plus main's round-11 additions (the overview's `see the fabric` line, FABRIC_PAGE's `aterm fabric` entry, and DRIVE_PAGE's `[--journal FILE]` on supervise/watch, report's `[--final | --messages]`, the `ledger` entry and the `--journal FILE` entry), checked against crates/aterm-link/src/hook.rs install_claude (self_test of every commands_of entry before anything is printed or written, `refused` exiting 2; an existing file without --merge printed and exit 1, nothing written; merge_into's write_backup `<file>.bak-<unix>`), check (`ok session=<sid> sock=<path>` or `not ok <reason>`, exit 0 either way), resolve (aterm_ctl::resolve_sock_for, then read_token_beside) and the module doc's two exit-2 verdicts; aterm-cli Verb::Fabric routed by crates/aterm/src/main.rs to aterm_link::fabric::main; crates/aterm-link/src/fabric.rs USAGE and parse_args (no argument; status the default; `--json` status-only; `tail [--bodies] [--from <offset>]`), probe_broker's connect_bounded + hello + attach + head query, TRAFFIC_ROWS = 10, exit_of's 0/1 and status_main's Off arm at 2; crates/aterm-agent/src/drive_cli.rs parse_sub (--journal on watch/supervise/ledger only, --format and --out on ledger only, --since's time grammar), ledger_verb and write_private's 0o600, supervise/report.rs render_view's ` view=<final|messages> kept=<n>` header, supervise/journal.rs's kind set and its create+append 0o600 open, supervise/blocks.rs NOTICE_HEADS, and supervise/ledger.rs parse_since (`T`, `Z`, `±HH:MM`); ONE SLIP FIXED at the merge, in main's prose: the `ledger` entry ended mid-sentence at `It only reads` — completed with DRIVE_HELP's own words (`It only reads: exit 0 even when a source was missing.`), true because ledger_verb's only Err paths are a missing worker, a parse error and a failed --out write, and supervise::gather returns a Ledger, never an Err; 2026-09-14 read of FABRIC_PAGE's rewritten TURNING IT ON opener (the `aterm fabric on` entry: idempotent per step and said per step, launchd / systemd --user, the rendezvous file every `aterm link` verb defaults from, arming the running instances, the 5 s self-post proof and exit 1, `--dry-run`, `off` keeping the identity, `doctor`, the script as a wrapper) against crates/aterm-link/src/enable.rs on (steps 1–11, PROOF_DEADLINE = 5 s, the already/would/done/FAILED verdicts, Service::default_here, arm_instances, prove's exit 1 through `on`), off (keeps node, secret and cap) and doctor (fix_for), crates/aterm-link/src/cli.rs dispatch (ls/glance/tui/mirror defaulted through enable) and tools/fabric-enable.sh (exec `$ATERM fabric on|off|status`); the by-hand steps beneath are unchanged and are still what `on` does (enable::node_grants is the page's eight grants, bridge_command carries --accept-from <node>); every line within the 92-column bound fabric_topic_turns_it_on_with_the_shipped_binary pins, whose needles now name `aterm fabric on|off|doctor` and `--dry-run`; no claim contradicted; 2026-09-14 read of FABRIC_PAGE's round-13 rows (the `status` synopsis with fabric_rtt_ms= / fabric_link_age_ms=, IS IT ON HERE?'s connected / stalled / disconnected entries, and the queued=1 clause naming `ERR fabric stalled`) against aterm-gui fabric.rs link_report, fabric_state, cmd_post and fabric_wait_refusal, control.rs fabric_status_line's reason tokens, and aterm-link bridge.rs ACK_DEADLINE = 5 s, RECONNECT_MIN = 100 ms / RECONNECT_MAX = 5 s, link_reason's tokens and LinkReport's report rules (change, more-than-2x, the 2 s refresh — never a timer); fabric_topic_is_reachable_and_honest_about_wake_paths pins the needles and refuses the pre-round-13 `NOT a statement about the bus` caveat, which is false now; no claim contradicted; 2026-09-14 read of FABRIC_PAGE's new WHO IS DOING WHAT — PRESENCE WITH MEANING section (feat/round-13-fabric-on, round 13 C) against crates/aterm-link/src/presence.rs Fields::tokens (attention= role= detail= phase= then context=<n>% only when read, then title=; Mode::Minimal writes attention= alone), phase_word (aterm_phase::worker_phase's name, `survey` only over an idle worker with the survey open), TAIL_ROWS = 40 and TITLE_CAP = 128 (user_title over title in Fields::read_meta), bridge.rs sample_presence (the screen read only when Slot::needs_screen sees the status revision move; meta and status on the 2 s roster round; nothing of the rows leaves read_screen but a word and a number) and Slot::due (change only, REPUBLISH_MIN 2 s), fabric.rs presence_from_config and cli.rs run (a `--presence` on the serve line wins, else [fabric] presence, else meta; a wrong value is meta with a word on stderr), aterm-gui session_status.rs quiet_after = 5 s (the `within about 7 s` bound: the classifier moves the revision 5 s after output stops, plus one roster round; measured 8 s in tests/r13_presence.rs with the republish window), cli.rs row (the ls columns) and fabric.rs render_text (ROLE DETAIL PHASE CTX); no claim contradicted; 2026-09-14 read of the merged FABRIC_PAGE at the rebase onto main's round-12 merge (the round-12 BEING WOKEN row and the round-13 rows in one page) and of the three fixes made at that read: `title=` is `meta set title`'s title else `-`, never the terminal's (aterm-link presence.rs read_meta takes `user_title=` alone and Fields::tokens writes `title=-` when it is unset); the turn's-end bound names its parts — 5 s of quiet (aterm-gui session_status.rs quiet_after), the next roster round (aterm-link bridge.rs ROSTER_REFRESH = 2 s), 8.1 s end to end measured in tests/r13_presence.rs — instead of `within about 7 s`; and the `stalled` row says a wait under `reason=starting` parks (aterm-gui fabric.rs wait_is_futile is false while the incarnation has not reported and its reason is starting; link_evidence moves the state to connected on the first `deliver` or `outbox sent`); no other claim contradicted; 2026-09-14 whole-manual audit fixes (34-finding pass): `fabric` is now a TOPICS entry (front-page index), the in-session brief carries one INBOX line inside its 90/7000 budget (86 lines, 6064 bytes after), the introspection page opens with a MAIL pointer to `aterm help fabric`; FABRIC_PAGE gained the two-watermark paragraph with dropped= and truncated= (read against fabric.rs cmd_inbox's listed vs handled marks, RING_CAP eviction reporting and the bridge's deliver line cut), `trust=screen` as the fourth value (read against control_verbs.rs's inbox row trust=<human|agent|relayed|screen>), `--session <sid>` on the mirror synopsis with its watermark hazard (read against aterm-link mirror.rs USAGE: default is EVERY session and the mirror runs `inbox seen` on the agent's behalf), step 0 mkdir/chmod 700 and step 3's `head -c32 /dev/urandom` secret creation (read against tools/fabric-enable.sh and cli.rs mint's SECRET_MIN = 32 refusal), and `aterm ctl help inbox` in SEE ALSO; every documented command spelled `aterm link …`; 2026-09-14 merge read of main over 28508563a: the merged prose is exactly round 13's FABRIC_PAGE rewrite (the `stalled` state and its reason= set, fabric_rtt_ms=/fabric_link_age_ms=, the PRESENCE WITH MEANING section, the `aterm fabric on` entry and the steps-`on`-takes sentence) plus main's audit additions (the `fabric` Topic and its tagline with EXTRA_PAGES no longer listing it, the introspection page's MAIL paragraph, the agent brief's INBOX line, the marks paragraph, trust=`screen`, step 0 and step 3's secret lines, the mirror's `--session <sid>` line and paragraph, and SEE ALSO's `aterm ctl help inbox`), checked against crates/aterm-gui/src/fabric.rs cmd_inbox (a non-peek call marks only the rows it RETURNED `listed`, per row — InboxRow::listed, whose doc says the listed state is NOT a watermark; `--peek` lists nothing), cmd_inbox_seen (seen = max(seen, id), then every row at or below id listed), the eviction's `dropped += 1` for a row above seen, the `truncated=1`/`len=` row tail (row.len > row.body.len()), TRUSTS = human|agent|relayed|screen and control_verbs.rs's inbox row; crates/aterm-link/src/cli.rs mint (SECRET_MIN = 32, a shorter secret and a repeated --secret-file refused) and mirror.rs Options::sessions (empty = every session; run_inbox issues `inbox seen`); render's Some(`fabric`) arm serving TOPICS' entry. THREE SLIPS IN MAIN'S PROSE FIXED at the merge, and the hash recorded here is the corrected page: the MAIL paragraph counted `three not-landed answers` where FABRIC_PAGE and cmd_post give FOUR (three of them queued); `TWO WATERMARKS` called the listed state a watermark a bare `inbox` advances, where it is per-row state on the rows the reply returned and `inbox seen` also lists the rows at or below the id (the paragraph, now TWO MARKS, says so); SEE ALSO's `both watermarks` likewise; 2026-09-14 read of the ten lines 2e3fa303d added to the pkg page (the `aterm pkg machine` and `aterm pkg machine apply` entries, carried by the merge of origin/main; `--diff` against this row's hash shows exactly those lines, and against origin/main's recorded row shows them plus the fabric-page prose this side read at d072e3afb) against crates/atpkg/src/cli.rs cmd_machine (bare: universal_control_state + doctor_line, then noindex::scan(home, DOCTOR_DEPTH, Budget::DOCTOR).counts(); apply: apply_machine_settings, ExitCode::SUCCESS on both arms, usage exit 2 otherwise), apply_machine_settings' place at the top of cmd_update_all_code/cmd_install_default_set_code/cmd_seed (after manager_enabled/layout/reconcile_links, which returns () and only narrates; pinned by machine_settings_apply_before_anything_can_fail_in_every_pass), machine.rs apply_universal_control/UNIVERSAL_CONTROL_REVERT (both keys, one line, printed by doctor_line when disabled), noindex.rs Pointer/ConfigNote (Symlink in a git checkout, Config elsewhere) and aterm-gui spawn_pkg_update_check (seed, then update); one slip fixed before this row: the `apply` entry said the migration leaves `a symlink left in place`, where noindex::apply_one leaves one only in a git checkout and writes [build] target-dir otherwise — it now says so, as the `noindex apply` entry above it already did; 2026-09-14 read of the PROVENANCE (macOS) bullet the merge 516d330df carried into the `aterm pkg update` page (shown by `xtask gate help-surfaces --diff`) against atpkg provenance.rs's measured facts and REMEDY, install.rs verify_and_stage's probe-file measurement and decide_tracked_stage/stage_for_store_with (InProcessRecorded and KeepRecorded both write the record beside the build; Refuse only under the env), lay.rs REFUSE_TRACKED_ENV/tracked_policy_of (unset is Allow) and lay_untracked, stage_helper.rs plan_helper (ExecOriginal for an untagged binary, CopyThenExec for a tagged free-standing one, CopyBundleThenExec for a tagged bundle executable), and doctor.rs's (5b) scan of every active build with provenance_bundle_line's fix; one slip fixed before this row: the bullet said the launchd job runs `a clean copy of its own binary`, where plan_helper runs an untagged binary in place and copies only a tagged one — it now says in place, or from a clean byte copy when the binary itself is tagged; 2026-09-15 read of round 15's key and deadline prose on the fabric page (shown by `xtask gate help-surfaces --diff`, matched at 7dcefc15f): EXACTLY ONCE against aterm-gui fabric.rs valid_key/KEY_MAX (1-64 of [A-Za-z0-9._:-]) and cmd_inbox (the in-flight `post` row prints `key=`), aterm-link bridge.rs drain_outbox (the key's sequence reserved in keys/<sid> before the post's pin and the publish, first wins, reused on a re-post; `dup=1` only when a key chose the sequence AND the broker deduped), state.rs KEYS_KEEP 4096, and vendored astream-broker store.rs (the (producer_id, producer_seq) dedup map rebuilt from the log on open, a duplicate answered with its original offset whatever its subject or body); the DEADLINE paragraph against note_deadline (ask/task only, and only for a record the post appended), settle_deadline_for (a reply settles only on the asker's own lane), settled_on_bus + expire_deadlines (the asker's lane read first, one `expired`) and deliver_record's late=1; and the queued-answers `key=` clause. ONE claim overstated and FIXED here: `a reply that comes after it arrives late=1` — the flag comes from the bridge's in-memory `expired` map (EXPIRED_KEEP, never persisted), so a relaunched bridge delivers a late reply unflagged; the paragraph now says so. No other claim contradicted; 2026-09-15 read of round 15's receipts and fetch-by-offset prose on the fabric page: `truncated=1` and NOTHING LOST against aterm-gui fabric.rs cmd_inbox's oldest_on_bus, inbox_get_at (the ring answers only a whole row, a cut or evicted one parks), deliver_fetched and aterm-link bridge.rs fetched_lines (chunks, cut at 256 KiB with len=) and fetch_failure (one `ERR no such record`); RECEIPTS against cmd_inbox_seen/owe_receipt (once per row per change, ask/task only), cmd_outbox's receipt lines and send_receipt/retire_receipt (a sequence pinned in acks/<sid>.<rid> before the publish); and the `[fabric] receipts` lines against enable.rs (writes `receipts = true` once), cli.rs (`--receipts`/`--no-receipts` win) and fabric.rs receipts_from_config; no claim contradicted; 2026-09-14 (feat/round-14-mail, round 14 D1/D2/D4) read of the --mail and task prose — DRIVE_HELP's synopsis lines, the watch, task and --mail entries and the journal's mail kind and report field (lib.rs); DRIVE_PAGE's watch, task, --mail and --journal entries (manual.rs); run.rs's module doc, SuperviseOpts::mail, Fold, MailIn, NoLane, Sink, the Review, StopAtReview, Lines and folded_event_line docs, supervise_mail, supervise_with, run_loop, watch_mail, watch_with, look's fold gate, hold_for_report, event_line_as, render_result_mail and the six mail test docs; the two skills' loop, watch, task and ledger paragraphs — against mail.rs Lane::serve (an inbox 1 --peek --meta baseline; await inbox since=<newest> kinds=<all nine> timeout <step>, re-armed on OK timeout, the kinds list dropped on ERR usage; inbox since=<id> --peek --meta, then inbox get <id> for the watched worker's report only; the MAIL line per row through the loop's sink; lane_off said once on any other ERR or a lapsed reconnect window), Lane::call's retry of a lost reply (pause doubling to pause_max, within reconnect), task() (post to=@sid kind=task [dl=<ms>] <text> from --inbox or @self, the offset from the OK's off=, one text --json read and `turn idle=600 timeout=2500 Inbox: task @<off>` only on Phase::Idle with submitted=1 as nudged, the inbox 1 baseline read BEFORE the post, await inbox … kinds=answer,report,ack re-armed on rows without re=<off>, the TIMEOUT line and EXIT_TIMEOUT), run.rs hold_for_report (pending drained; a delivery within the window folds at once, else recv_timeout to min(now + grace, deadline); Disconnected gives None so the line is as without the flag), look's Phase::Idle gate with the brief skipped only on Fold::Report, run_loop's thread::scope join (the lane returns within one mail_step) and drive_cli.rs parse_sub (--mail, --report-window and --idle-grace watch's and supervise's only; --inbox needs a leading @; --deadline, --wait and --no-nudge task's only), mail_needs_sid and task_opts (--wait bounded by --deadline, else --timeout); measured on the live worker s-1e918c4662a1b7b8bd43 (busy for the whole 25 s budget: TIMEOUT, the process ending 40 s in as the lane's parked wait was joined within its 20 s step, no MAIL lane off) and by the mock tests; no claim contradicted; 2026-09-14 (2026-09-15 UTC) read of the DRIVE_PAGE prose `xtask gate help-surfaces --diff` shows moved since main's read (matched at 53b9351bb): the watch synopsis and --mail sentence, the task entry, the --mail entry and --journal's kind/report fields — against aterm-agent supervise/mail.rs task() and Lane::serve, supervise/run.rs hold_for_report/MailIn::is_this_turns/render_result_mail, supervise/journal.rs JournalRecord::of_line (`MAIL` is kind mail; `report=`/`rows=` read off an EVENT turn line; to_json's trailing report field) and round 12's aterm-link hook.rs may_wake/accepted; one claim amended in this commit: the task entry's `a worker with round 12's wake hooks wants --no-nudge, its Stop hook wakes it` now says the hooks must list your sid in --accept-from (may_wake wakes for a human or a listed OWNER only; the bridge's --accept-from <N> is the node's list and governs demotion, not the wake); no other claim contradicted; 2026-09-15 read of FABRIC_PAGE's new --report-to paragraph in the Claude Code wake row against aterm-link hook.rs stop/report/recipient_check/claude_settings_for (the post before the wait; re= the newest unhandled task; 4 KiB with the marker inside; once per message by the content key in <state>/report/<sid>; the wake-budget charge; fail-open with the reason on stderr and exit 0; ` report-to=<sid>` on --check's ok line; the installer's refusal through self_test on a `not ok`); no claim contradicted; 2026-09-15 read of DRIVE_PAGE's --mail entry (the per-step screen read under a hold, a report attributed to its turn with --report-window bounding only one from before the turn was seen to begin, the lane's parked wait cut short) and the fabric page's --report-to paragraph (a regular file, re= unhandled or newer than the last report, the recipient's status first, the post's --wait landing with queued/dead/landed outcomes, --check's fabric status refusal) against aterm-agent supervise/run.rs hold_for_report/MailIn::is_this_turns/run_loop, supervise/mail.rs Lane::resync, and aterm-link hook.rs report/report_task/Landing/recipient_check/fabric_cannot_carry/last_assistant_text_within, by feat/round-14-mail on 2026-09-15 after the round-14 reviews; no claim contradicted; 2026-09-14 (2026-09-15 UTC) read of FABRIC_PAGE's --report-to paragraph as the prose --diff shows it moved — against aterm-link hook.rs stop/report/report_task/Landing::of/recipient_check/fabric_cannot_carry/self_test (an `ok` first line, else a refusal); one claim amended in this commit: `exit 0` for a report that failed is now `the wait as without the flag` (stop() runs the wait after a failed report, and a wake there is exit 2); the page gained the --accept-from sentence beside it (may_wake/accepted: a human or a listed OWNER sid); no other claim contradicted; 2026-09-14 (third audit lane, last merge): origin's 47198639f ('stabilize kitty motion and redraw trails') moved this file's prose (13 line(s)) without a row and this lane never touched the file; the union's hash is recorded at the merge, the read owed to that commit's author; 2026-09-15 read of 47198639f's two prose changes (PERMISSIONS step 3 and one GOTCHA, shown by `xtask gate help-surfaces --diff`) against control_privacy.rs: `full_disk_access=denied` is FdaState::Denied, the TCC.db open returning EPERM for the RUNNING identity, so `denied` is the effective check failing and not the Settings switch, and `running=` is the install-posture row the step tells the reader to match in Settings; the GOTCHA's bounded wait is finish_privacy_read on the control worker (one deadline of PRIVACY_READ_WAIT fixed before the first retry, Pending never extends it, a failed re-read returns the last Pending snapshot) and Denied is never derived from Pending; no claim contradicted; 2026-09-15: `aterm pkg machine` / `machine apply` entries re-read against atpkg cli.rs cmd_machine and noindex::apply_one by a reader prompted to refute — 'before the manager gate, the index and the network' (the store lock precedes it, and the refusal now says so), the symlink-vs-config-edit split, and the three apply answers; 2026-09-14 merge read of feat/round-14-mail over main: the merged prose is exactly round 14's change (DRIVE_PAGE's watch synopsis and --mail sentence, the task entry, the --mail entry and --journal's mail kind and report field; FABRIC_PAGE's --report-to and --accept-from paragraph: 4 hunks, 79 prose lines, shown by `xtask gate help-surfaces --diff` against main's read at b0691f788) plus upstream's change (the `aterm pkg machine apply` entry, the PROVENANCE bullet, PERMISSIONS step 3 and the privacy-wait GOTCHA: 4 hunks, 29 lines, shown against round 14's read at 6d66e6858, which the gate confirmed hashes to that row), on disjoint pages, checked against aterm-agent supervise/mail.rs task()/Lane::serve/MailRow::line/lane_off/DEFAULT_REPORT_WINDOW/DEFAULT_IDLE_GRACE/ANSWER_KINDS/NUDGE, supervise/run.rs MailIn::is_this_turns/hold_for_report/run_loop/look's Phase::Idle gate/folded_event_line/render_result_mail/EXIT_TIMEOUT/WAIT_STEP, supervise/journal.rs JournalRecord::of_line/to_json, drive_cli.rs parse_sub/mail_needs_sid/task_opts, aterm-link hook.rs stop/report/report_task/Landing::of/REPORT_WAIT_MS/trim_report/REPORT_MAX/report_key/Posted/recipient_check/fabric_cannot_carry/check/self_test/may_wake/accepted/last_assistant_text_within, bridge.rs accept_from (the node's list, demotion to note), atpkg cli.rs cmd_machine and cmd_update_all_code's ordering, provenance.rs QUARANTINE_XATTR, stage_helper.rs HelperPlan/plan_helper/plan_for_exe, install.rs decide_tracked_stage, lay.rs REFUSE_TRACKED_ENV/tracked_policy_of, doctor.rs's scan of the active builds, aterm-containment consent.rs classify_probe/FdaProbe::pending, aterm-gui control_privacy.rs finish_privacy_read/PRIVACY_READ_WAIT/`probe=`; four of upstream's words amended at the merge: the PROVENANCE bullet said a browser-downloaded app carries `com.apple.provenance` (it carries `com.apple.quarantine` and is tracked the same), said the clean copy is taken when the binary is tagged (plan_for_exe: tagged or quarantined), and said doctor lists every tagged bundle (the scan is over the active builds); the `machine apply` entry said `nothing needed to` where the verdict is `nothing changed — already applied, switched off in [machine], or a change that did not land`; no other claim contradicted; read of the PROVENANCE (macOS) bullet the merge 516d330df carried into the `aterm pkg update` page (shown by `xtask gate help-surfaces --diff`) against atpkg provenance.rs's measured facts and REMEDY, install.rs verify_and_stage's probe-file measurement and decide_tracked_stage/stage_for_store_with (InProcessRecorded and KeepRecorded both write the record beside the build; Refuse only under the env), lay.rs REFUSE_TRACKED_ENV/tracked_policy_of (unset is Allow) and lay_untracked, stage_helper.rs plan_helper (ExecOriginal for an untagged binary, CopyThenExec for a tagged free-standing one, CopyBundleThenExec for a tagged bundle executable), and doctor.rs's (5b) scan of every active build with provenance_bundle_line's fix; one slip fixed before this row: the bullet said the launchd job runs `a clean copy of its own binary`, where plan_helper runs an untagged binary in place and copies only a tagged one — it now says in place, or from a clean byte copy when the binary itself is tagged; 2026-09-14 (third audit lane, last merge): origin's 47198639f ('stabilize kitty motion and redraw trails') moved this file's prose (13 line(s)) without a row and this lane never touched the file; the union's hash is recorded at the merge, the read owed to that commit's author; 2026-09-15 read of 47198639f's two prose changes (PERMISSIONS step 3 and one GOTCHA, shown by `xtask gate help-surfaces --diff`) against control_privacy.rs: `full_disk_access=denied` is FdaState::Denied, the TCC.db open returning EPERM for the RUNNING identity, so `denied` is the effective check failing and not the Settings switch, and `running=` is the install-posture row the step tells the reader to match in Settings; the GOTCHA's bounded wait is finish_privacy_read on the control worker (one deadline of PRIVACY_READ_WAIT fixed before the first retry, Pending never extends it, a failed re-read returns the last Pending snapshot) and Denied is never derived from Pending; no claim contradicted; 2026-09-15: `aterm pkg machine` / `machine apply` entries re-read against atpkg cli.rs cmd_machine and noindex::apply_one by a reader prompted to refute — 'before the manager gate, the index and the network' (the store lock precedes it, and the refusal now says so), the symlink-vs-config-edit split, and the three apply answers; 2026-09-15 integration read: the starter contains 161 distinct commented keys after adding both machine settings. CONFIG_PAGE and starter_config_key_count_matches_the_manual now name that measured count. The complete `aterm pkg machine apply` command stays on one starter-comment line, matching the CLI handler and its disclosure regression; 2026-09-15 merge read of main (round 14) over origin/main 0504383fa: the merged page is exactly round 14's DRIVE_PAGE/FABRIC_PAGE additions plus upstream's one changed line (the starter aterm.toml now has 161 keys after the two machine settings), checked against crates/aterm-gui/src/cli.rs starter_config_key_count_matches_the_manual which asserts keys.len() == 161 and names this manual line; no slip found; 2026-09-15 merge read of feat/round-15-receipts over main: the merged prose is exactly round 15's change (FABRIC_PAGE's EXACTLY ONCE and A DEADLINE IS KEPT BY YOUR OWN BRIDGE paragraphs, `truncated=1`'s `inbox get @<off>` clause, the NOTHING LOST and RECEIPTS paragraphs, the queued-answers `key=` sentence and the `[fabric] receipts` lines: 4 hunks, shown by `xtask gate help-surfaces --diff` against main's read at aa4c990f0) plus upstream's change (round 14's DRIVE_PAGE watch/--mail/task/--journal entries and FABRIC_PAGE's --report-to and --accept-from paragraph, the `aterm pkg machine apply` entry, the PROVENANCE bullet, the 161 starter keys, PERMISSIONS step 3 and the privacy-wait GOTCHA, all read by main and shown against round 15's read at 030ba1788 — and f3a5467ed's two hunks, which main recorded no row for: the manual's front page and agent brief now end their title line at `\\n` so aterm_types::identity::ORIGIN_LINE (`by Andrew Yates · ALab · alab.systems`) prints under it before the blank line, read against overview_page's and agent_page's push_str sequence) plus ONE clause added at this merge to EXACTLY ONCE — a keyed re-post's ADDRESS is still resolved first, against the live roster, so one to a session that has gone is retired `ERR unroutable`, puts nothing on the bus and leaves the key naming its record — on disjoint pages, checked against aterm-link bridge.rs drain_outbox (the roster re-read once before any receipt or post is resolved; resolve_to before StateDir::key_seq; the non-routing arm retiring `off=- reason=` and clearing only the post's pin, never the key), refresh_sessions (a departed session's presence withdrawn as it is pruned), send_receipt/receipt_lane and answer_fetch, aterm-gui fabric.rs cmd_post/wait_landing (`ERR <reason> id=`), cmd_inbox/inbox_get_at/cmd_inbox_seen/owe_receipt, aterm-link hook.rs stop/report, atpkg cli.rs cmd_machine, control_privacy.rs, and the r15_receipts test a_keyed_re_post_to_a_session_whose_exit_line_never_arrives_is_unroutable_and_the_key_holds; 2026-09-15 read at the landing of the About byline fix, over 7b298a645 and 2ac21473f: the rust page's new trust version row read against trust_version_row and bounded_output (rustc -vV bounded at 2 s, the no-answer text shared with the probe rows; a trust: line with a release: line gets the compatible-with wording, a trust: line alone the short form, a missing or empty trust: line NONE) and its test on verbatim -vV texts; the NONE text's pointer to aterm pkg doctor holds, since doctor reports whether the stock names rustup resolves are the store's Trust tools and the managed version against the pin; no claim contradicted the code; 2026-09-15 read by fix/help-rust-targo-line of rust_page's prose it changed (shown by `xtask gate help-surfaces --diff`, matched at bc2e8e328: only this branch's edits) — rust_page's doc (the verify gate and `xtask gate lint` as discovery's users, the GUI claim dropped because aterm-gui does not link aterm-verify; DISCOVERY IS NOT WHAT A BARE `targo` RUNS; the page-spawned-only 2 s bound), the `gates' toolchain` line and its three-line label, `targo on PATH`, `gates' targo`, the `<name> on PATH` lines, the rustup `trust` no-answer line, the `PATH vs gates` AGREE / ONE BUILD, TWO DIRECTORIES / DIVERGENCE blocks and their consequence lines, REROUTE_DIR_MARKER, MAX_SHIM_HOPS, MAX_SHIM_BYTES, OnPath, sh_exec_target, first_executable_on_path, resolve_on_path, describe_on_path, build_of, Agreement, compare_toolchains and the five test docs — against aterm-verify toolchain.rs discover/discover_with_store (the rustup link at step 3 ahead of the store at step 4; every winner canonicalised; have_targo false for a refused dir), aterm-verify lib.rs Ctx::new and xtask gate.rs trust_toolchain/gate_lint_args (both discover under pinned_channel(repo root)), atpkg platform/mod.rs parse_sh_shim_target and MAX_SHIM_BYTES and sh_shim_content_env, reroute.rs DIR_MARKER_FILE and is_reroute_dir, doctor.rs's rustup `trust` lines (seam_line and the seam status row it speaks for), seam.rs's view doc (hard links under <prefix>/rustup), and by running the built `aterm help rust` from /Users//example (unpinned), this worktree (channel = \"trust\"), a scratch dir with PATH=/usr/bin:/bin, the stage2 tree first on PATH (AGREE) and TRUST_STAGE2_BIN at a hard-link view probe (ONE BUILD); `trustc takes its sysroot from the directory it runs from` measured with `--print sysroot` on the store, the stage2 tree and the view probe; five slips fixed before this row (the rustup line said NOT LINKED when rustup was not on PATH at all; the gates' label said `from an aterm checkout` where the page discovers under the cwd's pin; build_of's doc claimed a same-commit build could print two version lines; compare_toolchains' doc said a missing version never counts as agreement where one directory still does; the page doc said every subprocess is bounded where discovery's own --print sysroot checks are not); 2026-09-15 re-read after the review of d4156e63a (fix/help-rust-targo-line; only this branch's edits since) — rust_page's doc paragraph on the gates' pin, the `gates' toolchain` label (now `under aterm's own pin (channel = \"trust\")`, whatever this directory pins) and its NO-toolchain line, the shim-aware `gates' targo` line (`answers <line>`), the new `nothing to compare: the gates' `targo` does not resolve` arm, the SAME VERSION LINE, DIFFERENT FILES block, the doctor pointer now only on DIVERGENCE (`flags a rustup `trust` that is not atpkg's managed view`), ATERM_RUST_TOOLCHAIN, gates_pinned_channel, gates_toolchain, is_toolchain_bin, resolve_from, Agreement::SameLine, Side, compare_toolchains and the four new/rewritten test docs — against xtask gate.rs trust_toolchain and aterm-verify lib.rs Ctx::new (both pinned_channel(workspace root), which is the rust-toolchain.toml this binary is built from), toolchain.rs discover_with_store (`pinned.unwrap_or(\"trust\")` for the channel spellings; is_pinned_toolchain passing None and upstream channels), atpkg doctor.rs seam_line (None for a link to the view; warn for a link outside the prefix, note for one inside the store) and its section (9) conditions, and by running the built `aterm help rust` from /Users//example, this worktree, PATH=/usr/bin:/bin, a `stable`-pinned scratch dir (DIVERGENCE, was AGREE), TRUST_STAGE2_BIN=<prefix>/bin (AGREE through the shim, was ONE BUILD, TWO DIRECTORIES), a wrapper script echoing 07ec014cb first on PATH (SAME VERSION LINE, was ONE BUILD) and TRUST_STAGE2_BIN at the hard-link view probe (ONE BUILD, no doctor pointer), and `aterm pkg doctor` here printing the rustup `trust` NOT-the-managed-store warn; one slip fixed before this row (the SAME VERSION LINE text said the sysroot sits `beside` the bin where it is the directory above); 2026-09-15 re-read of the merged rust_page after rebasing this branch over 7b298a645/2ac21473f/16dd50611: the upstream `trust version` row (trust_version_row over bounded_output, which bounded_first_line now wraps) and this branch's gates'/PATH lines render together in the order the code writes them (checked by running the built `aterm help rust` from /Users//example), neither side's text refers to a line the other removed, and the row now carries both reads instead of the rebase's auto-merge silently keeping only this branch's; 2026-09-15 read of the routed-shim prose fix/tippy-view-shims added (the ShimHop/Route/Refusal/ShimGuard/ShimScript docs, sh_shim_script, sh_guard_line, sh_quoted_word, guard_refusal, resolve_from, describe_on_path, EXEC_ROOTS_DIR, MAX_SHIM_HOPS, is_toolchain_bin, the rust_page doc and the exec-root note beside the rustup-view note) against atpkg's platform::sh_shim_content_routed (the guard `[ ! -h 'R' ] && [ 'R' -ef 'S' ] && exec 'R' \"$@\"` ahead of the unchanged store exec, R rendered three times by sh_shim_quote), parse_sh_shim_target (first trimmed `exec '` line, rfind of the `\"$@\"` tail), compat::route_for_shim/ensure_root (roots under <prefix>/compat/trust/<n>, bin/ hard links with rustc as trustc's inode, lib/ mirrored so root/bin is a toolchain bin) and sh's test builtin (-h is lstat of the last component, -ef stats both following links and compares dev+ino): the page now reports R with the store file S it is when the guard holds and S with the refused R and which half failed otherwise, checked against /bin/sh itself by routed_shims_resolve_to_where_the_shell_execs (the fixture targo prints its $0 for a hard link, a copy, a symlink and nothing) and end to end by the page test's exec-root scenario; two slips fixed before this row (the refusal line said `its atpkg exec root <file>` for what is the root's FILE, and the exec-root note said the directory's files are the build's with rustc IS trustc, which only doctor's Deep walk proves — it now says they were laid so and names doctor); 2026-09-16 read of the two prose changes the merge b3def0952/3f232dad1 brought (shown by `xtask gate help-surfaces --diff`, matched at 45ef863f8e2ad454): the PKG page's shell-integration sentence now naming `~/.bash_profile`, and the PROVENANCE bullet's new `[packages] tracked_install = \"refuse\"` clause. Read against atpkg hooks.rs ensure_rc_sources_hooks (the four rows are `.zshrc`/`.bashrc`/`.bash_profile`/`.config/fish/config.fish`, each sourcing `00-atpkg.{zsh,bash,bash,fish}`; symlink_metadata-is_err skips, so nothing is CREATED; canonicalize + protected::under_protected_root is the consent fence the same sentence claims) and its module doc's measured bash table (`bash -l -i` reads `.bash_profile` alone, `bash -i` reads `.bashrc` alone, macOS /bin/bash 3.2.57, 2026-09-15) — so the page's `what a login bash reads on macOS - Terminal.app, ssh` is the measurement, not a guess; and against lay.rs tracked_policy_of/REFUSE_TRACKED_ENV/ALLOW_TRACKED_ENV with config.rs PackagesConfig::tracked_install + tracked_install_of (two spellings, `record` and `refuse`; anything else is dropped at load with a warning), which give exactly the precedence the page states: a non-empty env var wins for that one run, an unset or empty one defers to the key, neither is Allow. Both claims hold; NO false claim found in this file this round 2026-09-15: re-read the two macOS-permissions paragraphs (the agent brief's MACOS FILE PERMISSIONS block and `aterm help permissions`) against protected_folder_names(), which renders aterm_containment::consent::Folder::ALL \u{2014} the roster gained `app-data`, so both now say `the places macOS protects` and gloss the last name as every other app's own data instead of listing app data separately; the Full Disk Access sentence, the do-not list and the tool list are unchanged. 2026-09-16 audit-3 read: the pkg page's MACHINE SETTINGS block and the `noindex apply` entry re-read against the shipped ordering — the settings are applied FIRST by every pass (cli::main_entry's edge call above mutator_store_lock, then apply_machine_settings_once in each body), not `apply --all` at the end; Universal Control is re-written whenever the host is not fully disabled, not once (machine::apply_universal_control); the pass scans with Budget::DOCTOR while `apply --all` uses Budget::VERB, which the entry now states; the SKIP_DIRS folders are named as never walked; and the two non-change answers (`machine settings not applied —`, `machine settings failed —`) are documented. 2026-09-16 (audit 3, after the rebase onto 39636319f): the pkg page's MACHINE SETTINGS block and the `noindex apply` entry re-read against the merged code — applied FIRST by every pass AND above the store lock at main_entry's edge, Universal Control re-written whenever the host is not fully disabled, the pass's Budget::DOCTOR vs `apply --all`'s Budget::VERB, the SKIP_DIRS folders named as never walked, and the two non-change answers; upstream's own machine work in this window (the orphan stand-down, MachineConfig::inert, the lstat-free walk) changes none of those sentences.; 2026-09-16 read of the two lines added to the PLUMBING `link` entry (this tree, shown by `xtask gate help-surfaces --diff`) against atpkg cli.rs cmd_link (calls reassert_rustup_seam after a successful link when program == seam::SEAM_PROGRAM) and cmd_unlink (reassert_rustup_seam when store/trust/current stands, else detach_rustup_seams), seam::view_source (Linked only when the checkout has bin/ and lib/ — `a sysroot` — else LinkedNoSysroot, which attach refuses and records, leaving the view as it stands) and seam::attach/reassert; `points rustup's trust at the checkout` is the view <prefix>/rustup/trust becoming symbolic links into the checkout under an unchanged ~/.rustup/toolchains/trust entry, which is what `rustup run trust` resolves; no claim contradicted the code; 2026-09-16 re-read of the `aterm help rust` exec-root and rustup-view text changed by the clone construction (the where-notes, ShimGuard/Refusal/refusal_words, sh_guard_line, guard_refusal, describe_on_path, is_toolchain_bin, MAX_SHIM_HOPS) against atpkg platform::sh_shim_content_routed (the marker guard `[ ! -h R ] && [ -f R ] && [ -x R ] && [ -f S ] && [ ! -h M ] && [ -f M ]`), compat::route_for_shim/ROOT_MARKER/root_marker, seam::lay_view and clone::clone_file/is_clone_of; the guard's decisions measured under /bin/sh, zsh, bash, ksh and dash over nine shapes; routed_shims_resolve_to_where_the_shell_execs runs /bin/sh on each refusal; and the atpkg page's rustup sentence, which still named store/trust/current, corrected to the view; no claim left that a root or the store view is hard links",
    ),
    (
    ...
    (
        ...
        "ac6aad1b13a77fd8",
        "2026-09-17",
        "re-read on 2026-09-13 by drift sweep of 2026-09-13 (lane xtask); three enumerations had drifted twice and are now derived from the dispatch and the verify driver; 2026-09-13 release candidate: the ONE prose change since that read is the doc comment this cut added to `WRAPPED_BY_VERIFY_SH` (it was `tippy`-warned as never used because only `the_wrapped_list_is_exactly_what_the_verify_driver_calls` and the two doc paragraphs that link it read it; the const is now `#[cfg_attr(not(test), allow(dead_code))]` and the comment says why). Read against its only consumer, that test, and against the verify driver's stage list it is checked against; the list's six verbs are unchanged; no claim contradicted; 2026-09-14 drift sweep (lane xtask, 17 claims checked over the group): re-read against the code that moved under it since this row's anchor commit; every candidate went to a verifier prompted to REFUTE it — ONE slip fixed before this row, and it was made THIS SESSION: `run`'s doc enumerated the arguments `gate help-surfaces` reads as `--diff PATH`, and `--drift` landed beside it hours earlier. The sweep caught the commit that created it; 2026-09-16 read of the two literals this change added (`/usr/bin/env`, `bash`, shown by `xtask gate help-surfaces --diff`) against run_repo_guards: every guard is `#!/usr/bin/env bash` and resolves itself from `$0`, which `bash <script>` sets to the script's path, so the interpreter launch changes only whether a tagged script file tracks the guard (measured: exec → tracked, `bash <script>` → untracked); the `$ …` transcript line names the interpreter first, and no fixture pins it; no claim contradicted; 2026-09-17 read of the prose this cut changed — `shim_refusal`'s rewritten doc, the new `host_shim_refusal`'s doc, the `gate_cells` paragraph about a row naming the reader's own triple, the `cshim` row NOT APPLIED note the per-cell shim loop now prints, and the docs and assertion messages of the two new tests and the rewritten tail of `a_build_script_shim_cannot_be_written_loosely` — against their handlers: `parse_cell_policy`, which now calls only the machine-independent `shim_refusal`; the per-cell loop in `gate_cells`, which marks the row live and then asks `host_shim_refusal(&cell.triple, &host)`; and the end-of-run `cshim_row_used` audit that the stays-in-the-policy-file half of that note is a claim about. MEASURED on m17-tower, where `linux` is the NATIVE cell and the two `x86_64-unknown-linux-gnu` `cshim` rows name the host: `gate cells --cell win` went from `COULD NOT RUN — tools/cross-cell-gate.tsv:89: refusing the `cshim` row for `ring`` to reaching the `win` cell and reporting a real Windows type error, and the unnarrowed run prints the NOT-APPLIED note for both rows, calls neither dead, and leaves the `linux` cell GREEN at 266/266 with the real build scripts run. No claim contradicted; 2026-09-17 read of the prose this cut added — `gate_cells`'s new `TWO PASSES PER CELL` paragraph, the docs of `parse_member_names`, `cell_test_selection`, `test_pass_exemption`, `TestPass`, `TestPassJob` and `run_cell_test_pass`, the run lines the new step 6d prints, the TESTS column's header note, the test-coverage clause appended to the final verdict sentence, and the docs and assertion messages of the three new tests — against their handlers: step 6d in `gate_cells` (which asks `test_pass_exemption` first, then skips when the library pass already failed, then runs the pass and owes an artifact per member), `run_cell_test_pass` (the `--workspace --exclude` selection, the `profile.test` artifact filter, the empty-selection refusal) and the table/verdict printers. The `usage: xtask gate cells [--cell NAME]…` string was re-read against its own parser and is unchanged and still true: `--cell` is repeatable and an empty list means all five of `aterm_forge::resolve::default_cells()`. MEASURED on m17-tower: with the `aterm-verify` and `cfg-if` fixes of this commit reverted, `gate cells --cell win` exits 1 with `TEST-TARGET-ERROR(33)`; with them in place it exits 0 at `70/70`, `--cell linux` prints the host exemption and `--cell wasm-cpu` the wasm one, both `-` in the TESTS column. No claim contradicted",
    ),
    (
