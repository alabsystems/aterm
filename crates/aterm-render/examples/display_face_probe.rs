...
fn main() {
    ...
    let family = args.next().unwrap_or_else(|| "display:chunky".into());
    let px: f32 = args.next().unwrap_or_else(|| "16".into()).parse().unwrap();
    let text = args.next().unwrap_or_else(|| "immediate, wm.".into());

    let mut r =
        Renderer::from_system_with_family(Some(&family), px, Theme::default()).expect("renderer");
    ...
    let ruler: String = (0..w)
        .map(|x| if x % cw == 0 { '+' } else { '-' })
        .collect();
    println!("|{ruler}|");
}
