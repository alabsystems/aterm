...
`v0.1.0` begins aterm's public version series. It is produced as an immutable,
single-commit source snapshot from the private development repository. The
private development line and its historical labels are not public releases.

The publication transform makes only reviewable boundary changes:

- sets the public Cargo workspace and first-party lockfile records to `0.1.0`;
...
Some compatibility and regression tests retain historical development-lineage
strings such as `v0.56`. Those strings are fixtures, not public version claims.
The authoritative public package version is the root Cargo workspace version
and `aterm --version`, both checked as `0.1.0` during staging.
